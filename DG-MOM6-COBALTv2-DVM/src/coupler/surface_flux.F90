...
!     thv_surf= t_surf0                        ! surface virtual (potential) T -- just for testing tun off the q_surf
     ...
     u_dif = - u_atm ! u_surf - u_atm    mpoupon                ! velocity components relative to surface
     v_dif = - v_atm ! v_surf - v_atm    mpoupon
  endwhere

  if(alt_gustiness) then
     do i = 1, size(avail)
        if (.not.avail(i)) cycle
        w_atm(i) = 10.0 ! max(sqrt(u_dif(i)**2 + v_dif(i)**2), gust_const) ! mpoupon
        ! derivatives of surface wind w.r.t. atm. wind components
        if(w_atm(i) > gust_const) then
           dw_atmdu(i) = 1.0 ! u_dif(i)/w_atm(i) ! mpoupon
           dw_atmdv(i) = 0.0 ! v_dif(i)/w_atm(i) ! mpoupon
        else
           dw_atmdu(i) = 0.0
           dw_atmdv(i) = 0.0
        endif
     ...
     where(avail)
        w_atm = 10.0 ! sqrt(u_dif*u_dif + v_dif*v_dif + w_gust*w_gust) ! mpoupon
        ! derivatives of surface wind w.r.t. atm. wind components
        dw_atmdu = 1.0 ! u_dif/w_atm ! mpoupon
        dw_atmdv = 0.0 ! v_dif/w_atm ! mpoupon
     endwhere
  endif
  ...
  where (avail)
     ...
     ! stresses
     rho_drag   = drag_m * rho
     flux_u     = 1.29*0.0018*abs(u_dif)*u_dif  ! mpoupon rho_drag * u_dif   ! zonal      component of stress (Nt/m**2)
     flux_v     = 1.29*0.0018*abs(v_dif)*v_dif  ! mpoupon rho_drag * v_dif   ! meridional component of stress
...
real   , intent(inout), dimension(:) :: cd, ch, ce, ustar, bstar
  ...
  if(ncar_ocean_flux_orig) then
      ...
      do i=1,size(u_del(:))
         if (avail(i)) then
             tv = t(i)*(1+0.608*q(i))
             u = 10.0 !max(u_del(i), 0.5);                            ! 0.5 m/s floor on wind (undocumented NCAR) (mpoupon)
             ...
             do j=1,n_itts                                           ! Monin-Obukhov iteration
                cd_rt = sqrt(cd(i));
                ustar(i) = 0.01 !cd_rt*u;     mpoupon                              ! L-Y eqn. 7a
      ...
      do i=1,size(u_del(:))
         if (avail(i)) then
             ...
             do j=1,n_itts                                           ! Monin-Obukhov iteration
                cd_rt = sqrt(cd(i));
                ustar(i) = 0.01 ! mpoupon ! cd_rt*u;                                   ! L-Y eqn. 7a
                tstar    = (ch(i)/cd_rt)*(t(i)-ts(i));                ! L-Y eqn. 7b
                qstar    = (ce(i)/cd_rt)*(q(i)-qs(i));                ! L-Y eqn. 7c
                bstar(i) = grav*(tstar/tv+qstar/(q(i)+1/0.608));
                zeta     = vonkarm*bstar(i)*z(i)/(ustar(i)*ustar(i)); ! L-Y eqn. 8a
