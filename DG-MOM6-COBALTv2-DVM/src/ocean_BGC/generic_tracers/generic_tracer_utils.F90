...
!-----------------------------------------------------------------------
  ...
  type g_tracer_type
     ...
     ! An 3D field for implicit vertical diffusion only and vertical particle sinking only ! mpoupon
     real, _ALLOCATABLE, dimension(:,:,:)  :: vert_sinkingc    _NULL ! mpoupon
     real, _ALLOCATABLE, dimension(:,:,:)  :: vert_diffusionc  _NULL ! mpoupon
     ...
     integer :: diag_id_vert_sinkingc = -1, diag_id_vert_diffusionc = -1 ! mpoupon
...
contains
  ...
  subroutine g_tracer_init(g_tracer)
    ...
    if(g_tracer%prog) then
       ...
       !liao
       !mpoupon
       allocate(g_tracer%vert_sinkingc(isd:ied,jsd:jed,nk))
       g_tracer%vert_sinkingc(:,:,:) = 0.0
       allocate(g_tracer%vert_diffusionc(isd:ied,jsd:jed,nk))
       g_tracer%vert_diffusionc(:,:,:) = 0.0
       !mpoupon
    endif
...
!            implementation    = 'ocmip2',                                                     &
  ...
  subroutine g_tracer_register_diag(g_tracer)
   ...
   !liao
    ...
    !liao
    !mpoupon
    string=trim(g_tracer%alias) // trim("_vert_sinkingc")
    g_tracer%diag_id_vert_sinkingc = g_register_diag_field(g_tracer%package_name, &
         trim(string),                 &
         g_tracer_com%axes(1:3),       &
         g_tracer_com%init_time,       &
         'Active/Gravitational vertical sinking of ' //trim(g_tracer%alias),   &
         trim('mole/kg/s'),                  &
         missing_value = -1.0e+20)

    string=trim(g_tracer%alias) // trim("_vert_diffusionc")
    g_tracer%diag_id_vert_diffusionc = g_register_diag_field(g_tracer%package_name, &
         trim(string),                 &
         g_tracer_com%axes(1:3),       &
         g_tracer_com%init_time,       &
         'Vertical diffusion (only) concentration of ' // trim(g_tracer%alias), &
         trim('mole/kg/s'),                  &
         missing_value = -1.0e+20)
    !mpoupon
...
!    if(present(ocean_restart_file)) ocean_restart_file  = g_tracer_com%ocean_restart_file
  ...
  subroutine g_tracer_get_3D(g_tracer_list,name,member,array_ptr)
    ...
    !mpoupon
    case ('vert_sinkingc')
       array_ptr => g_tracer%vert_sinkingc
    case ('vert_diffusionc')
       array_ptr => g_tracer%vert_diffusionc
    !mpoupon
    case default 
  ...
  subroutine g_tracer_get_3D_val(g_tracer_list,name,member,array,isd,jsd,ntau,positive)
    ...
    !mpoupon
    case ('vert_sinkingc')
       array(:,:,:) = g_tracer%vert_sinkingc(:,:,:)
    case ('vert_diffusionc')
       array(:,:,:) = g_tracer%vert_diffusionc(:,:,:)
    !mpoupon
    case default 
       call mpp_error(FATAL, trim(sub_name)//": Not a known member variable: "//trim(member))   
    end select
  ...
  subroutine g_tracer_set_3D(g_tracer_list,name,member,array,isd,jsd,ntau)
    ...
    !mpoupon
    case ('vert_sinkingc')
       g_tracer%vert_sinkingc  = array
    case ('vert_diffusionc')
       g_tracer%vert_diffusionc  = array
    !mpoupon
    case default 
       call mpp_error(FATAL, trim(sub_name)//": Not a known member variable: "//trim(member))   
    end select
...
!     local parameters
  ...
  subroutine g_tracer_send_diag(g_tracer_list,model_time,tau)
    ...
    do  
       ...
       !liao
       !mpoupon
       if (g_tracer%diag_id_vert_sinkingc .gt. 0 .and._ALLOCATED(g_tracer%vert_sinkingc)) then
          used = g_send_data(g_tracer%diag_id_vert_sinkingc,g_tracer%vert_sinkingc(:,:,:), model_time,&
               rmask = g_tracer_com%grid_tmask(:,:,:),&
               is_in=g_tracer_com%isc, js_in=g_tracer_com%jsc, ks_in=1,&
               ie_in=g_tracer_com%iec, je_in=g_tracer_com%jec,ke_in=g_tracer_com%nk)
       endif

       if (g_tracer%diag_id_vert_diffusionc .gt. 0 .and._ALLOCATED(g_tracer%vert_diffusionc)) then
          used = g_send_data(g_tracer%diag_id_vert_diffusionc,g_tracer%vert_diffusionc(:,:,:), model_time,&
               rmask = g_tracer_com%grid_tmask(:,:,:),&
               is_in=g_tracer_com%isc, js_in=g_tracer_com%jsc, ks_in=1,&
               ie_in=g_tracer_com%iec, je_in=g_tracer_com%jec,ke_in=g_tracer_com%nk)
       endif
  ...
  subroutine g_tracer_vertdiff_G(g_tracer, h_old, ea, eb, dt, kg_m2_to_H, m_to_H, tau, mom)
    ...
    if (g_tracer%diag_id_vdiffuse_impl .gt. 0 .or. g_tracer%diag_id_vdiffusec_impl .gt. 0 .or. &     ! mpoupon added
            g_tracer%diag_id_vert_sinkingc .gt. 0 .or. g_tracer%diag_id_vert_diffusionc .gt. 0) then ! mpoupon added
      if (present(mom)) then
    ...
    do j=g_tracer_com%jsc,g_tracer_com%jec ; do i=g_tracer_com%isc,g_tracer_com%iec 
       ...
       if (g_tracer_com%grid_tmask(i,j,1) > 0.5) then
          ...
          if (do_diagnostic) then
             ! Sinking only
             do k=1,nz
                a(k)= -max(sink(k),0.0)/h_old(i,j,k)
                c(k)= min(sink(k+1),0.0)/h_old(i,j,k)
                b(k)=  (h_old(i,j,k)+max(sink(k+1),0.0)-min(sink(k),0.0))/h_old(i,j,k)
                f_old(k)= g_tracer%field(i,j,k,tau)
             enddo

             call tridag_solver_Press_et_al(a,b,c,f_old,g_tracer%vert_sinkingc(i,j,:),nz)

             ! Vertical diffusion only
             do k=1,nz
                a(k)= -ea(i,j,k)/h_old(i,j,k)
                c(k)= -eb(i,j,k)/h_old(i,j,k)
                b(k)=  (h_old(i,j,k)+eb(i,j,k)+ea(i,j,k))/h_old(i,j,k)
                f_old(k)= g_tracer%field(i,j,k,tau)
             enddo

             call tridag_solver_Press_et_al(a,b,c,f_old,g_tracer%vert_diffusionc(i,j,:),nz)
    ...
    if (do_diagnostic) then
      do j = g_tracer_com%jsc, g_tracer_com%jec
         do i = g_tracer_com%isc, g_tracer_com%iec
            do k = 1, g_tracer_com%nk
               ! mpoupon
               g_tracer%vert_sinkingc(i,j,k) = g_tracer_com%grid_tmask(i,j,k) * &
                    (g_tracer%vert_sinkingc(i,j,k) - g_tracer%vdiffuse_impl(i,j,k))/ dt
               g_tracer%vert_diffusionc(i,j,k) = g_tracer_com%grid_tmask(i,j,k) * &
                    (g_tracer%vert_diffusionc(i,j,k) - g_tracer%vdiffuse_impl(i,j,k))/ dt
               ! mpoupon
