...
namelist /generic_COBALT_nml/ do_14c, co2_calc, debug, do_nh3_atm_ocean_exchange, scheme_nitrif, &
  ...
  type phytoplankton
     ...
     real, ALLOCATABLE, dimension(:,:)  :: &
          ...
          def_fe_bw_100,    &
          irrlim_bw_100,    &
          wc_vert_int_maxgrowth_lightlim_n,  & ! mpoupon - plankton dynamics
          wc_vert_int_maxgrowth_nutlim_n,    & ! mpoupon - plankton dynamics
          wc_vert_int_maxgrowth_n,   & ! mpoupon - plankton dynamics
          wc_vert_int_jprod_n,       & ! mpoupon - plankton dynamics
          wc_vert_int_jresp_n,       & ! mpoupon - plankton dynamics
          wc_vert_int_jvirloss_n,    & ! mpoupon - plankton dynamics
          wc_vert_int_jaggloss_n,    & ! mpoupon - plankton dynamics
          wc_vert_int_jexuloss_n,    & ! mpoupon - plankton dynamics
          wc_vert_int_jzloss_n,      & ! mpoupon - plankton dynamics
          wc_vert_int_jphyto_n,      & ! mpoupon - plankton dynamics
          wc_vert_int_f_n              ! mpoupon - plankton dynamics
     real, ALLOCATABLE, dimension(:,:,:)  :: &
          def_fe      , &
          def_p       , &
          f_fe        , &
     ...
     integer ::            &
          ...
          id_jexuloss_n_100   = -1, &
          id_f_n_100          = -1, &
          id_wc_vert_int_maxgrowth_lightlim_n    = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_maxgrowth_nutlim_n      = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_maxgrowth_n = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jprod_n     = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jresp_n     = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jvirloss_n  = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jaggloss_n  = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jexuloss_n  = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jzloss_n    = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jphyto_n    = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_f_n         = -1, & ! mpoupon - plankton dynamics
          id_sfc_f_n          = -1, &
          id_sfc_chl          = -1, &
          id_sfc_def_fe       = -1, &
          id_sfc_felim        = -1, &
  ...
  type zooplankton
    real ::  &
          ...
          swim_stop_o2,     & ! oxygen concentration limit for swimming
    ...
    real, ALLOCATABLE, dimension(:,:)  :: &
          ...
          f_n_100,          &
          z_mig,            &  ! mpoupon
          wc_vert_int_jingest_n,   & ! mpoupon - plankton dynamics
          wc_vert_int_jmetabo_n,   & ! mpoupon - plankton dynamics
          wc_vert_int_jmetabo_bas_n,   & ! mpoupon - plankton dynamics
          wc_vert_int_jmetabo_dig_n,   & ! mpoupon - plankton dynamics
          wc_vert_int_jmetabo_swm_n,   & ! mpoupon - plankton dynamics
          wc_vert_int_jprod_n,     & ! mpoupon - plankton dynamics
          wc_vert_int_jprodpos_n,     & ! mpoupon - plankton dynamics
          wc_vert_int_jmort_n,     & ! mpoupon - plankton dynamics
          wc_vert_int_jzloss_n,    & ! mpoupon - plankton dynamics
          wc_vert_int_jhploss_n,   & ! mpoupon - plankton dynamics
          wc_vert_int_jzoo_n,      & ! mpoupon - plankton dynamics
          wc_vert_int_f_n            ! mpoupon - plankton dynamics
    real, ALLOCATABLE, dimension(:,:,:) :: &
          f_n,              & ! zooplankton biomass
          f_gut_n,          & ! zooplankton n gut content ! mpoupon
          f_gut_p,          & ! zooplankton p gut content ! mpoupon
          f_gut_fe,         & ! zooplankton fe gut content ! mpoupon
          f_gut_si,         & ! zooplankton si gut content ! mpoupon
          f_met_n,          & ! zooplankton metabolites ! mpoupon
          assim_eff,        & ! zooplankton assimilation efficiency
          jclear_gut_n,     & ! mpoupon
          jprod_gut_n,      & ! mpoupon
          jclear_gut_p,     & ! mpoupon
          jprod_gut_p,      & ! mpoupon
          jclear_gut_fe,    & ! mpoupon
          jprod_gut_fe,     & ! mpoupon
          jclear_gut_si,    & ! mpoupon
          jprod_gut_si,     & ! mpoupon
          jclear_met_n,     & ! mpoupon
          jprod_met_n,      & ! mpoupon 
          lim_nut_n_ingestion, & ! mpoupon
          jmetabo_n,        & ! mpoupon
          ...
          jingest_n_lim,    & ! Total ingestion of limiting n ! mpoupon
    ...
    real, dimension(:,:,:), pointer :: &
           p_vmove, &
           p_vmove_gut, & ! mpoupon
           p_vmove_gut_p, & ! mpoupon
           p_vmove_gut_fe, & ! mpoupon
           p_vmove_gut_si, & ! mpoupon
           p_vmove_met    ! mpoupon 
    integer ::          &
          id_jzloss_n       = -1, &
          id_jzloss_p       = -1, &
          ...
          id_f_n_100        = -1, &
          id_z_mig          = -1, &
          id_wc_vert_int_jingest_n    = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jmetabo_n    = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jmetabo_bas_n = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jmetabo_dig_n = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jmetabo_swm_n = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jprod_n      = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jprodpos_n   = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jmort_n      = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jzloss_n     = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jhploss_n    = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_jzoo_n       = -1, & ! mpoupon - plankton dynamics
          id_wc_vert_int_f_n          = -1, & ! mpoupon - plankton dynamics
          id_assim_eff         = -1, &
          id_jclear_gut_n      = -1, &
          id_jprod_gut_n       = -1, &
  ...
  type generic_COBALT_type
     ...
     real  ::          &
          ...
          kirr_hp,          & ! Half saturation response for light limitation
          ...
          hp_phi_vis          ! fraction of visual higher pred. (Added by mpoupon) 
     ...
     real, dimension(:,:,:), ALLOCATABLE ::  &
          ...
          jprod_nh4,&
          jprod_nh4_plus_btm,&
          jprod_nh4_conso, &              ! mpoupon
          jprod_nh4_resp, &               ! mpoupon
          jprod_nh4_passive, &            ! mpoupon
          jprod_ndet_passive, &           ! mpoupon
          jprod_don_passive, &            ! mpoupon
          jprod_nh4_active_respiration, & ! mpoupon
          jprod_ndet_active_digestion, &  ! mpoupon
          jprod_ndet_active_mortality, &  ! mpoupon
          jprod_don_active_excretion, &   ! mpoupon
          jprod_po4,&
          net_phyto_resp,&
          det_jzloss_n,&
          det_jzloss_p,&
          ...
          hp_vis_lim, &
          hp_jingest_n,&
          hp_jingest_n_active,  & ! mpoupon
          hp_jingest_n_passive, & ! mpoupon
          hp_jingest_p,&
          hp_jingest_fe,&
          hp_jingest_sio2,&
          irr_inst,&
...
!==============================================================================================================
     ...
     integer               ::          &
          ...
          id_jprod_nh4_conso = -1, &    ! mpoupon
          id_jprod_nh4_resp  = -1, &    ! mpoupon
          id_jprod_nh4_passive = -1, &  ! mpoupon
          id_jprod_ndet_passive = -1, & ! mpoupon
          id_jprod_don_passive  = -1, & ! mpoupon
          id_jprod_nh4_active_respiration = -1, &  ! mpoupon
          id_jprod_ndet_active_digestion  = -1, &  ! mpoupon
          id_jprod_ndet_active_mortality  = -1, &  ! mpoupon
          id_jprod_don_active_excretion   = -1, &  ! mpoupon
          id_net_phyto_resp = -1,      &
          id_det_jzloss_n  = -1,       &
          id_det_jzloss_p  = -1,       &
          id_det_jzloss_fe = -1,       &
          ...
          id_jndet         = -1,       &
          id_jndetc        = -1,       &   ! mpoupon
          id_jsldonc       = -1,       &   ! mpoupon
          id_jldonc        = -1,       &   ! mpoupon
          id_jsrdonc       = -1,       &   ! mpoupon
          id_jndic         = -1,       &   ! mpoupon
          id_jnsmc         = -1,       &   ! mpoupon
          id_jnlgc         = -1,       &   ! mpoupon
          id_jnbactc       = -1,       &   ! mpoupon
          id_jnsmzc        = -1,       &   ! mpoupon
          id_jnmdzc        = -1,       &   ! mpoupon
          id_jnlgzc        = -1,       &   ! mpoupon
          id_jnvmmdzc      = -1,       &   ! mpoupon
          id_jnvmlgzc      = -1,       &   ! mpoupon
          id_jcadet_aragc  = -1,       &   ! mpoupon
          id_jcadet_calcc  = -1,       &   ! mpoupon
          id_jnh4_plus_btm = -1,       &
          id_jno3denit_wc  = -1,       &
          id_juptake_no3amx = -1,      &
          id_juptake_nh4amx = -1,      &
          ...
          id_tot_layer_int_doc = -1,   & ! mpoupon
          id_tot_layer_int_dic = -1,   & ! mpoupon
          id_tot_layer_int_poc = -1,   & ! mpoupon
          id_tot_layer_int_pic = -1,   & ! mpoupon
...
!    real                                  :: conversion
    ...
    cobalt%id_rho_dzt = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
    ...
    ! mpoupon
    vardesc_temp = vardesc("jingest_lim_n_Smz","Ingestion of nitrogen by small zooplankton, layer integral",&
                           'h','L','s','mol N m-2 s-1','f')
    zoo(1)%id_jingest_n_lim = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jingest_lim_n_Mdz","Ingestion of nitrogen by medium-sized zooplankton, layer integral",&
                           'h','L','s','mol N m-2 s-1','f')
    zoo(2)%id_jingest_n_lim = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jingest_lim_n_Lgz","Ingestion of nitrogen by large zooplankton, layer integral",&
                           'h','L','s','mol N m-2 s-1','f')
    zoo(3)%id_jingest_n_lim = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jingest_lim_n_vmMdz","Ingestion of nitrogen by vertically migrating medium zooplankton, layer integral",&
                           'h','L','s','mol N m-2 s-1','f')
    zoo(4)%id_jingest_n_lim = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jingest_lim_n_vmLgz","Ingestion of nitrogen by vertically migrating large zooplankton, layer integral",&
                           'h','L','s','mol N m-2 s-1','f')
    zoo(5)%id_jingest_n_lim = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
    ...
    !
    !  MPOUPON NOTE: ADD DIAGNOSTIC jprod and jclear gut and met here!
    !
...
! CAS added a "plus_btm" version of jprod_nh4 to use for the remoc CMIP variable
    ...
    !
    ! Carbon pump diagnostics (added by mpoupon)
    !

    vardesc_temp = vardesc("jprod_nh4_conso","NH4 consumption by biology layer integral",'h','L','s','mol NH4 m-2 s-1','f')
    cobalt%id_jprod_nh4_conso = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_nh4_resp","NH4 production by all respiration - dic pump - layer integral",'h','L','s','mol NH4 m-2 s-1','f')
    cobalt%id_jprod_nh4_resp = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_nh4_passive","NH4 production by non swimming organisms - layer integral",'h','L','s','mol NH4 m-2 s-1','f')
    cobalt%id_jprod_nh4_passive = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_ndet_passive","NDET production by non swimming organisms - layer integral",'h','L','s','mol NH4 m-2 s-1','f')
    cobalt%id_jprod_ndet_passive = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_don_passive","DON production by non swimming organisms - layer integral",'h','L','s','mol NH4 m-2 s-1','f')
    cobalt%id_jprod_don_passive = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_nh4_active_respiration","NH4 production by swimming organisms - layer integral",'h','L','s','mol NH4 m-2 s-1','f')
    cobalt%id_jprod_nh4_active_respiration = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_ndet_active_digestion","NDET production by swimming organisms digestion - layer integral",'h','L','s','mol NH4 m-2 s-1','f')
    cobalt%id_jprod_ndet_active_digestion = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_ndet_active_mortality","NDET production by swimming organisms death - layer integral",'h','L','s','mol NH4 m-2 s-1','f')
    cobalt%id_jprod_ndet_active_mortality = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_don_active_excretion","DON production by swimming organisms - layer integral",'h','L','s','mol NH4 m-2 s-1','f')
    cobalt%id_jprod_don_active_excretion = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
...
!liao
    ...
    vardesc_temp = vardesc("hp_vis_lim","Visual limitation of higher predators",'h','L','s','dimensionless','f')
    cobalt%id_hp_vis_lim = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
    ...
    vardesc_temp = vardesc("irr_inst_cw","Instantaneous Light in clear water",'h','L','s','W m-2','f')
    cobalt%id_irr_inst_cw = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
    ...
    ! mpoupon
    vardesc_temp = vardesc("tot_layer_int_doc","DOC boxwise layer integral",'h','L','s','mol m-2','f')
    cobalt%id_tot_layer_int_doc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
    
    vardesc_temp = vardesc("tot_layer_int_poc","POC boxwise layer integral",'h','L','s','mol m-2','f')
    cobalt%id_tot_layer_int_poc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("tot_layer_int_dic","DIC boxwise layer integral",'h','L','s','mol m-2','f')
    cobalt%id_tot_layer_int_dic = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("tot_layer_int_pic","PIC boxwise layer integral",'h','L','s','mol m-2','f')
    cobalt%id_tot_layer_int_pic = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
    ! mpoupon
...
! CAS: Added diagnostic for diatom NPP in top 100m for CMIP
    ...
    !
    ! Plankton integrated fluxes - mpoupon - plankton dynamics 
    
    vardesc_temp = vardesc("maxgrowth_n_lightlim_sm_wc_vi","Water column integrated small phytoplankton nitrogen max growth - light limited",'h','1','s','mol m-2 s-1','f')
phyto(SMALL)%id_wc_vert_int_maxgrowth_lightlim_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("maxgrowth_n_lightlim_lg_wc_vi","Water column integrated large phytoplankton nitrogen max growth - light limited",'h','1','s','mol m-2 s-1','f')
phyto(LARGE)%id_wc_vert_int_maxgrowth_lightlim_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("maxgrowth_n_lightlim_di_wc_vi","Water column integrated diazotroph nitrogen max growth - light limited",'h','1','s','mol m-2 s-1','f')
phyto(DIAZO)%id_wc_vert_int_maxgrowth_lightlim_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("maxgrowth_n_nutlim_sm_wc_vi","Water column integrated small phytoplankton nitrogen max growth - nutrient limited",'h','1','s','mol m-2 s-1','f')
phyto(SMALL)%id_wc_vert_int_maxgrowth_nutlim_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("maxgrowth_n_nutlim_lg_wc_vi","Water column integrated large phytoplankton nitrogen max growth - nutrient limited",'h','1','s','mol m-2 s-1','f')
phyto(LARGE)%id_wc_vert_int_maxgrowth_nutlim_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("maxgrowth_n_nutlim_di_wc_vi","Water column integrated diazotroph nitrogen max growth - nutrient limited",'h','1','s','mol m-2 s-1','f')
phyto(DIAZO)%id_wc_vert_int_maxgrowth_nutlim_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("maxgrowth_n_sm_wc_vi","Water column integrated small phytoplankton nitrogen max growth",'h','1','s','mol m-2 s-1','f')
phyto(SMALL)%id_wc_vert_int_maxgrowth_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("maxgrowth_n_lg_wc_vi","Water column integrated large phytoplankton nitrogen max growth",'h','1','s','mol m-2 s-1','f')
phyto(LARGE)%id_wc_vert_int_maxgrowth_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("maxgrowth_n_di_wc_vi","Water column integrated diazotroph nitrogen max growth",'h','1','s','mol m-2 s-1','f')
phyto(DIAZO)%id_wc_vert_int_maxgrowth_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprod_n_sm_wc_vi","Water column integrated small phytoplankton nitrogen production",'h','1','s','mol m-2 s-1','f')
phyto(SMALL)%id_wc_vert_int_jprod_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprod_n_lg_wc_vi","Water column integrated large phytoplankton nitrogen production",'h','1','s','mol m-2 s-1','f')
phyto(LARGE)%id_wc_vert_int_jprod_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprod_n_di_wc_vi","Water column integrated diazotroph nitrogen production",'h','1','s','mol m-2 s-1','f')
phyto(DIAZO)%id_wc_vert_int_jprod_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jresp_n_sm_wc_vi","Water column integrated small phytoplankton nitrogen respiration",'h','1','s','mol m-2 s-1','f')
phyto(SMALL)%id_wc_vert_int_jresp_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jresp_n_lg_wc_vi","Water column integrated large phytoplankton nitrogen respiration",'h','1','s','mol m-2 s-1','f')
phyto(LARGE)%id_wc_vert_int_jresp_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jresp_n_di_wc_vi","Water column integrated diazotroph nitrogen respiration",'h','1','s','mol m-2 s-1','f')
phyto(DIAZO)%id_wc_vert_int_jresp_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jvirloss_n_sm_wc_vi","Water column integrated small phytoplankton nitrogen viral lysis",'h','1','s','mol m-2 s-1','f')
phyto(SMALL)%id_wc_vert_int_jvirloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jvirloss_n_lg_wc_vi","Water column integrated large phytoplankton nitrogen viral lysis",'h','1','s','mol m-2 s-1','f')
phyto(LARGE)%id_wc_vert_int_jvirloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jvirloss_n_di_wc_vi","Water column integrated diazotroph nitrogen viral lysis",'h','1','s','mol m-2 s-1','f')
phyto(DIAZO)%id_wc_vert_int_jvirloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jaggloss_n_sm_wc_vi","Water column integrated small phytoplankton nitrogen aggregation",'h','1','s','mol m-2 s-1','f')
phyto(SMALL)%id_wc_vert_int_jaggloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jaggloss_n_lg_wc_vi","Water column integrated large phytoplankton nitrogen aggregation",'h','1','s','mol m-2 s-1','f')
phyto(LARGE)%id_wc_vert_int_jaggloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jaggloss_n_di_wc_vi","Water column integrated diazotroph nitrogen aggregation",'h','1','s','mol m-2 s-1','f')
phyto(DIAZO)%id_wc_vert_int_jaggloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jexuloss_n_sm_wc_vi","Water column integrated small phytoplankton nitrogen exudation",'h','1','s','mol m-2 s-1','f')
phyto(SMALL)%id_wc_vert_int_jexuloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jexuloss_n_lg_wc_vi","Water column integrated large phytoplankton nitrogen exudation",'h','1','s','mol m-2 s-1','f')
phyto(LARGE)%id_wc_vert_int_jexuloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jexuloss_n_di_wc_vi","Water column integrated diazotroph nitrogen exudation",'h','1','s','mol m-2 s-1','f')
phyto(DIAZO)%id_wc_vert_int_jexuloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzloss_n_sm_wc_vi","Water column integrated small phytoplankton nitrogen loss to zooplankton",'h','1','s','mol m-2 s-1','f')
phyto(SMALL)%id_wc_vert_int_jzloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzloss_n_lg_wc_vi","Water column integrated large phytoplankton nitrogen loss to zooplankton",'h','1','s','mol m-2 s-1','f')
phyto(LARGE)%id_wc_vert_int_jzloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzloss_n_di_wc_vi","Water column integrated diazotroph nitrogen loss to zooplankton",'h','1','s','mol m-2 s-1','f')
phyto(DIAZO)%id_wc_vert_int_jzloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jphyto_n_sm_wc_vi","Water column integrated small phytoplankton nitrogen accumulation",'h','1','s','mol m-2 s-1','f')
phyto(SMALL)%id_wc_vert_int_jphyto_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jphyto_n_lg_wc_vi","Water column integrated large phytoplankton nitrogen accumulation",'h','1','s','mol m-2 s-1','f')
phyto(LARGE)%id_wc_vert_int_jphyto_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jphyto_n_di_wc_vi","Water column integrated diazotroph nitrogen accumulation",'h','1','s','mol m-2 s-1','f')
phyto(DIAZO)%id_wc_vert_int_jphyto_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("nsm_wc_vi","Water column integrated small phytoplankton nitrogen biomass",'h','1','s','mol m-2 s-1','f')
phyto(SMALL)%id_wc_vert_int_f_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("nlg_wc_vi","Water column integrated large phytoplankton nitrogen biomass",'h','1','s','mol m-2 s-1','f')
phyto(LARGE)%id_wc_vert_int_f_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("ndi_wc_vi","Water column integrated diazotroph nitrogen biomass",'h','1','s','mol m-2 s-1','f')
phyto(DIAZO)%id_wc_vert_int_f_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
    
    vardesc_temp = vardesc("jingest_n_smz_wc_vi","Water column integrated small zooplankton nitrogen ingestion",'h','1','s','mol m-2 s-1','f')
    zoo(1)%id_wc_vert_int_jingest_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jingest_n_mdz_wc_vi","Water column integrated medium zooplankton nitrogen ingestion",'h','1','s','mol m-2 s-1','f')
    zoo(2)%id_wc_vert_int_jingest_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jingest_n_lgz_wc_vi","Water column integrated large zooplankton nitrogen ingestion",'h','1','s','mol m-2 s-1','f')
    zoo(3)%id_wc_vert_int_jingest_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jingest_n_vmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton nitrogen ingestion",'h','1','s','mol m-2 s-1','f')
    zoo(4)%id_wc_vert_int_jingest_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jingest_n_vmlgz_wc_vi","Water column integrated vertically migration large zooplankton nitrogen ingestion",'h','1','s','mol m-2 s-1','f')
    zoo(5)%id_wc_vert_int_jingest_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
    init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_n_smz_wc_vi","Water column integrated small zooplankton nitrogen metabolism",'h','1','s','mol m-2 s-1','f')
zoo(1)%id_wc_vert_int_jmetabo_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_n_mdz_wc_vi","Water column integrated medium zooplankton nitrogen metabolism",'h','1','s','mol m-2 s-1','f')
zoo(2)%id_wc_vert_int_jmetabo_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_n_lgz_wc_vi","Water column integrated large zooplankton nitrogen metabolism",'h','1','s','mol m-2 s-1','f')
zoo(3)%id_wc_vert_int_jmetabo_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_n_vmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton nitrogen metabolism",'h','1','s','mol m-2 s-1','f')
zoo(4)%id_wc_vert_int_jmetabo_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_n_vmlgz_wc_vi","Water column integrated vertically migration large zooplankton nitrogen metabolism",'h','1','s','mol m-2 s-1','f')
zoo(5)%id_wc_vert_int_jmetabo_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_bas_n_smz_wc_vi","Water column integrated small zooplankton nitrogen metabolism (basal)",'h','1','s','mol m-2 s-1','f')
zoo(1)%id_wc_vert_int_jmetabo_bas_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_bas_n_mdz_wc_vi","Water column integrated medium zooplankton nitrogen metabolism (basal)",'h','1','s','mol m-2 s-1','f')
zoo(2)%id_wc_vert_int_jmetabo_bas_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_bas_n_lgz_wc_vi","Water column integrated large zooplankton nitrogen metabolism (basal)",'h','1','s','mol m-2 s-1','f')
zoo(3)%id_wc_vert_int_jmetabo_bas_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_bas_n_vmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton nitrogen metabolism (basal)",'h','1','s','mol m-2 s-1','f')
zoo(4)%id_wc_vert_int_jmetabo_bas_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_bas_n_vmlgz_wc_vi","Water column integrated vertically migration large zooplankton nitrogen metabolism (basal)",'h','1','s','mol m-2 s-1','f')
zoo(5)%id_wc_vert_int_jmetabo_bas_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_dig_n_smz_wc_vi","Water column integrated small zooplankton nitrogen metabolism (digestion)",'h','1','s','mol m-2 s-1','f')
zoo(1)%id_wc_vert_int_jmetabo_dig_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_dig_n_mdz_wc_vi","Water column integrated medium zooplankton nitrogen metabolism (digestion)",'h','1','s','mol m-2 s-1','f')
zoo(2)%id_wc_vert_int_jmetabo_dig_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_dig_n_lgz_wc_vi","Water column integrated large zooplankton nitrogen metabolism (digestion)",'h','1','s','mol m-2 s-1','f')
zoo(3)%id_wc_vert_int_jmetabo_dig_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_dig_n_vmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton nitrogen metabolism (digestion)",'h','1','s','mol m-2 s-1','f')
zoo(4)%id_wc_vert_int_jmetabo_dig_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_dig_n_vmlgz_wc_vi","Water column integrated vertically migration large zooplankton nitrogen metabolism (digestion)",'h','1','s','mol m-2 s-1','f')
zoo(5)%id_wc_vert_int_jmetabo_dig_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_swm_n_smz_wc_vi","Water column integrated small zooplankton nitrogen metabolism (swim)",'h','1','s','mol m-2 s-1','f')
zoo(1)%id_wc_vert_int_jmetabo_swm_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_swm_n_mdz_wc_vi","Water column integrated medium zooplankton nitrogen metabolism (swim)",'h','1','s','mol m-2 s-1','f')
zoo(2)%id_wc_vert_int_jmetabo_swm_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_swm_n_lgz_wc_vi","Water column integrated large zooplankton nitrogen metabolism (swim)",'h','1','s','mol m-2 s-1','f')
zoo(3)%id_wc_vert_int_jmetabo_swm_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_swm_n_vmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton nitrogen metabolism (swim)",'h','1','s','mol m-2 s-1','f')
zoo(4)%id_wc_vert_int_jmetabo_swm_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmetabo_swm_n_vmlgz_wc_vi","Water column integrated vertically migration large zooplankton nitrogen metabolism (swim)",'h','1','s','mol m-2 s-1','f')
zoo(5)%id_wc_vert_int_jmetabo_swm_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)


vardesc_temp = vardesc("jprod_n_smz_wc_vi","Water column integrated small zooplankton nitrogen production",'h','1','s','mol m-2 s-1','f')
zoo(1)%id_wc_vert_int_jprod_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprod_n_mdz_wc_vi","Water column integrated medium zooplankton nitrogen production",'h','1','s','mol m-2 s-1','f')
zoo(2)%id_wc_vert_int_jprod_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprod_n_lgz_wc_vi","Water column integrated large zooplankton nitrogen production",'h','1','s','mol m-2 s-1','f')
zoo(3)%id_wc_vert_int_jprod_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprod_n_vmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton nitrogen production",'h','1','s','mol m-2 s-1','f')
zoo(4)%id_wc_vert_int_jprod_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprod_n_vmlgz_wc_vi","Water column integrated vertically migrating large zooplankton nitrogen production",'h','1','s','mol m-2 s-1','f')
zoo(5)%id_wc_vert_int_jprod_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprodpos_n_smz_wc_vi","Water column integrated small zooplankton positive nitrogen production",'h','1','s','mol m-2 s-1','f')
zoo(1)%id_wc_vert_int_jprodpos_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprodpos_n_mdz_wc_vi","Water column integrated medium zooplankton positive nitrogen production",'h','1','s','mol m-2 s-1','f')
zoo(2)%id_wc_vert_int_jprodpos_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprodpos_n_lgz_wc_vi","Water column integrated large zooplankton positive nitrogen production",'h','1','s','mol m-2 s-1','f')
zoo(3)%id_wc_vert_int_jprodpos_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprodpos_n_vmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton positive nitrogen production",'h','1','s','mol m-2 s-1','f')
zoo(4)%id_wc_vert_int_jprodpos_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jprodpos_n_vmlgz_wc_vi","Water column integrated vertically migrating large zooplankton positive nitrogen production",'h','1','s','mol m-2 s-1','f')
zoo(5)%id_wc_vert_int_jprodpos_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmort_n_smz_wc_vi","Water column integrated small zooplankton nitrogen mortality",'h','1','s','mol m-2 s-1','f')
zoo(1)%id_wc_vert_int_jmort_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmort_n_mdz_wc_vi","Water column integrated medium zooplankton nitrogen mortality",'h','1','s','mol m-2 s-1','f')
zoo(2)%id_wc_vert_int_jmort_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmort_n_lgz_wc_vi","Water column integrated large zooplankton nitrogen mortality",'h','1','s','mol m-2 s-1','f')
zoo(3)%id_wc_vert_int_jmort_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmort_n_vmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton nitrogen mortality",'h','1','s','mol m-2 s-1','f')
zoo(4)%id_wc_vert_int_jmort_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jmort_n_vmlgz_wc_vi","Water column integrated vertically migration large zooplankton nitrogen mortality",'h','1','s','mol m-2 s-1','f')
zoo(5)%id_wc_vert_int_jmort_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzloss_n_smz_wc_vi","Water column integrated small zooplankton nitrogen loss to zooplankton",'h','1','s','mol m-2 s-1','f')
zoo(1)%id_wc_vert_int_jzloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzloss_n_mdz_wc_vi","Water column integrated medium zooplankton nitrogen loss to zooplankton",'h','1','s','mol m-2 s-1','f')
zoo(2)%id_wc_vert_int_jzloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzloss_n_lgz_wc_vi","Water column integrated large zooplankton nitrogen loss to zooplankton",'h','1','s','mol m-2 s-1','f')
zoo(3)%id_wc_vert_int_jzloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzloss_n_vmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton nitrogen loss to zooplankton",'h','1','s','mol m-2 s-1','f')
zoo(4)%id_wc_vert_int_jzloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzloss_n_vmlgz_wc_vi","Water column integrated vertically migration large zooplankton nitrogen loss to zooplankton",'h','1','s','mol m-2 s-1','f')
zoo(5)%id_wc_vert_int_jzloss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jhploss_n_smz_wc_vi","Water column integrated small zooplankton nitrogen loss to higher predators",'h','1','s','mol m-2 s-1','f')
zoo(1)%id_wc_vert_int_jhploss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jhploss_n_mdz_wc_vi","Water column integrated medium zooplankton nitrogen loss to higher predators",'h','1','s','mol m-2 s-1','f')
zoo(2)%id_wc_vert_int_jhploss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jhploss_n_lgz_wc_vi","Water column integrated large zooplankton nitrogen loss to higher predators",'h','1','s','mol m-2 s-1','f')
zoo(3)%id_wc_vert_int_jhploss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jhploss_n_vmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton nitrogen loss to higher predators",'h','1','s','mol m-2 s-1','f')
zoo(4)%id_wc_vert_int_jhploss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jhploss_n_vmlgz_wc_vi","Water column integrated vertically migration large zooplankton nitrogen loss to higher predators",'h','1','s','mol m-2 s-1','f')
zoo(5)%id_wc_vert_int_jhploss_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzoo_n_smz_wc_vi","Water column integrated small zooplankton nitrogen accumulation",'h','1','s','mol m-2 s-1','f')
zoo(1)%id_wc_vert_int_jzoo_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzoo_n_mdz_wc_vi","Water column integrated medium zooplankton nitrogen accumulation",'h','1','s','mol m-2 s-1','f')
zoo(2)%id_wc_vert_int_jzoo_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzoo_n_lgz_wc_vi","Water column integrated large zooplankton nitrogen accumulation",'h','1','s','mol m-2 s-1','f')
zoo(3)%id_wc_vert_int_jzoo_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzoo_n_vmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton nitrogen accumulation",'h','1','s','mol m-2 s-1','f')
zoo(4)%id_wc_vert_int_jzoo_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("jzoo_n_vmlgz_wc_vi","Water column integrated vertically migration large zooplankton nitrogen accumulation",'h','1','s','mol m-2 s-1','f')
zoo(5)%id_wc_vert_int_jzoo_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

 
vardesc_temp = vardesc("nsmz_wc_vi","Water column integrated small zooplankton nitrogen biomass",'h','1','s','mol m-2','f')
zoo(1)%id_wc_vert_int_f_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("nmdz_wc_vi","Water column integrated medium zooplankton nitrogen biomass",'h','1','s','mol m-2','f')
zoo(2)%id_wc_vert_int_f_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("nlgz_wc_vi","Water column integrated large zooplankton nitrogen biomass",'h','1','s','mol m-2','f')
zoo(3)%id_wc_vert_int_f_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("nvmmdz_wc_vi","Water column integrated vertically migrating medium zooplankton nitrogen biomass",'h','1','s','mol m-2','f')
zoo(4)%id_wc_vert_int_f_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("nvmlgz_wc_vi","Water column integrated vertically migrating large zooplankton nitrogen biomass",'h','1','s','mol m-2','f')
zoo(5)%id_wc_vert_int_f_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),& 
 init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)


vardesc_temp = vardesc("z_mig_smz","Migration depth - small zooplankton",'h','1','s','m','f')
zoo(1)%id_z_mig = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("z_mig_mdz","Migration depth -  medium zooplankton",'h','1','s','m','f')
zoo(2)%id_z_mig = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("z_mig_lgz","Migration depth - large zooplankton",'h','1','s','m','f')
zoo(3)%id_z_mig = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("z_mig_vmmdz","Migration depth - vertically migrating medium zooplankton",'h','1','s','m','f')
zoo(4)%id_z_mig = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
     init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

vardesc_temp = vardesc("z_mig_vmlgz","Migration depth - vertically migrating large zooplankton",'h','1','s','m','f')
zoo(5)%id_z_mig = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
 init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
    !
...
! mpoupon 
    vardesc_temp = vardesc("jndetc","NDET source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jndetc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jsldonc","SLDON source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jsldonc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jldonc","LDON source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jldonc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jsrdonc","SRDON source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jsrdonc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jndic","NDI source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jndic = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jnlgc","NLG source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jnlgc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jnsmc","NSM source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jnsmc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jnbactc","NBACT source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jnbactc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jnlgzc","NLGZ source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jnlgzc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jnmdzc","NMDZ source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jnmdzc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jnvmlgzc","NVMLGZ source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jnvmlgzc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jnvmmdzc","NVMMDZ source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jnvmmdzc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jnsmzc","NSMZ source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jnsmzc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jcadet_aragc","Aragonite source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jcadet_aragc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jcadet_calcc","Calcite source concentration",'h','L','s','mol m-3 s-1','f')
    cobalt%id_jcadet_calcc = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
! mpoupon
...
!    call g_tracer_add_param('o2_2_nfix', cobalt%o2_2_nfix, (118.0+3.0/(5.0+3.0)*(150.0-118.0))/16.0) ! mol O2 mol N-1
    ...
    call g_tracer_add_param('imax_vmmdz',zoo(4)%imax, 0.68 / sperd)              ! s-1
    call g_tracer_add_param('imax_vmlgz',zoo(5)%imax, 0.28 / sperd)              ! s-1
    ...
    !-----------------------------------------------------------------------
    !
    call g_tracer_add_param('dvm_I_thresh_smz',zoo(1)%dvm_I_thresh, 0.0001)       ! W m-2
    call g_tracer_add_param('dvm_I_thresh_mdz',zoo(2)%dvm_I_thresh, 0.0001)       ! W m-2
    call g_tracer_add_param('dvm_I_thresh_lgz',zoo(3)%dvm_I_thresh, 0.0001)       ! W m-2
    call g_tracer_add_param('dvm_I_thresh_vmmdz',zoo(4)%dvm_I_thresh, 0.001)      ! W m-2
    call g_tracer_add_param('dvm_I_thresh_vmlgz',zoo(5)%dvm_I_thresh, 0.0001)     ! W m-2
    ...
    call g_tracer_add_param('swim_max_vmmdz',zoo(4)%swim_max, 0.06)               ! m s-1
    call g_tracer_add_param('swim_max_vmlgz',zoo(5)%swim_max, 0.08)               ! m s-1
    call g_tracer_add_param('swim_ref_smz',zoo(1)%swim_ref, 0.1)                  ! m s-1
    call g_tracer_add_param('swim_ref_mdz',zoo(2)%swim_ref, 0.1)                  ! m s-1
    call g_tracer_add_param('swim_ref_lgz',zoo(3)%swim_ref, 0.1)                  ! m s-1
    call g_tracer_add_param('swim_ref_vmmdz',zoo(4)%swim_ref, 0.12)               ! m s-1
    call g_tracer_add_param('swim_ref_vmlgz',zoo(5)%swim_ref, 0.16)               ! m s-1
    ...
    call g_tracer_add_param('swim_stop_o2_smz',zoo(1)%swim_stop_o2, 60.0e-6)    ! moles O2 kg-2
    call g_tracer_add_param('swim_stop_o2_mdz',zoo(2)%swim_stop_o2, 60.0e-6)    ! moles O2 kg-2
    call g_tracer_add_param('swim_stop_o2_lgz',zoo(3)%swim_stop_o2, 60.0e-6)    ! moles O2 kg-2
    call g_tracer_add_param('swim_stop_o2_vmmdz',zoo(4)%swim_stop_o2, 60.0e-6)  ! moles O2 kg-2
    call g_tracer_add_param('swim_stop_o2_vmlgz',zoo(5)%swim_stop_o2, 60.0e-6)  ! moles O2 kg-2
    !
    !-----------------------------------------------------------------------
    ...
    call g_tracer_add_param('bresp_vmmdz',zoo(4)%bresp, 0.008*0.9 / sperd)           ! s-1
    call g_tracer_add_param('bresp_vmlgz',zoo(5)%bresp, 0.0032*0.9 / sperd)          ! s-1
    !call g_tracer_add_param('k_clear_gut_smz',zoo(1)%k_clear_gut, 0.0 / sperd)         ! s-1
    !call g_tracer_add_param('k_clear_gut_mdz',zoo(2)%k_clear_gut, 0.0 / sperd)         ! s-1
    !call g_tracer_add_param('k_clear_gut_lgz',zoo(3)%k_clear_gut, 0.0 / sperd)         ! s-1
    call g_tracer_add_param('k_clear_gut_vmmdz',zoo(4)%k_clear_gut, 8.0 / sperd)        ! s-1
    call g_tracer_add_param('k_clear_gut_vmlgz',zoo(5)%k_clear_gut, 8.0 / sperd)        ! s-1
    !call g_tracer_add_param('k_temp_gut_smz',zoo(1)%k_temp_gut, 0.0 / sperd)           ! s-1
    !call g_tracer_add_param('k_temp_gut_mdz',zoo(2)%k_temp_gut, 0.0 / sperd)           ! s-1
    !call g_tracer_add_param('k_temp_gut_lgz',zoo(3)%k_temp_gut, 0.0 / sperd)           ! s-1
    call g_tracer_add_param('k_temp_gut_vmmdz',zoo(4)%k_temp_gut, 4.32 / sperd)         ! s-1
    call g_tracer_add_param('k_temp_gut_vmlgz',zoo(5)%k_temp_gut, 4.32 / sperd)         ! s-1
    !call g_tracer_add_param('k_clear_met_smz',zoo(1)%k_clear_met, 0.0 / sperd)         ! s-1
    !call g_tracer_add_param('k_clear_met_mdz',zoo(2)%k_clear_met, 0.0 / sperd)         ! s-1
    !call g_tracer_add_param('k_clear_met_lgz',zoo(3)%k_clear_met, 0.00 / sperd)        ! s-1
    call g_tracer_add_param('k_clear_met_vmmdz',zoo(4)%k_clear_met, 1.0 / sperd)        ! s-1
    call g_tracer_add_param('k_clear_met_vmlgz',zoo(5)%k_clear_met, 1.0 / sperd)        ! s-1
    ...
    call g_tracer_add_param('phi_aresp_vmmdz',zoo(4)%phi_aresp, 0.3)             ! dimensionless
    call g_tracer_add_param('phi_aresp_vmldz',zoo(5)%phi_aresp, 0.3)             ! dimensionless
    ...
    call g_tracer_add_param('assim_eff_max_vmmdz',zoo(4)%assim_eff_max, 0.7)     ! dimensionless
    call g_tracer_add_param('assim_eff_max_vmlgz',zoo(5)%assim_eff_max, 0.7)     ! dimensionless
    ...
    call g_tracer_add_param('assim_eff_min_vmmdz',zoo(4)%assim_eff_min, 0.7)     ! dimensionless
    call g_tracer_add_param('assim_eff_min_vmlgz',zoo(5)%assim_eff_min, 0.7)     ! dimensionless    
    ...
    call g_tracer_add_param('kae_vmsmz',zoo(4)%kae, 1.0e+10)                     ! moles N kg-1
    call g_tracer_add_param('kae_vmlgz',zoo(5)%kae, 1.0e+10)                     ! moles N kg-1
    !
    !----------------------------------------------------------------------
    ...
    call g_tracer_add_param('phi_det_vmmdz',zoo(4)%phi_det, 0.7)             ! dimensionless
    call g_tracer_add_param('phi_det_vmlgz',zoo(5)%phi_det, 1.0)             ! dimensionless
    ...
    call g_tracer_add_param('phi_ldon_vmmdz',zoo(4)%phi_ldon, 0.7*0.3)       ! dimensionless
    ...
    call g_tracer_add_param('phi_ldop_vmmdz',zoo(4)%phi_ldop, 0.65*0.3)      ! dimensionless
    ...
    call g_tracer_add_param('phi_srdon_vmmdz',zoo(4)%phi_srdon, 0.1*0.3)     ! dimensionless
    ...
    call g_tracer_add_param('phi_srdop_vmmdz',zoo(4)%phi_srdop, 0.15*0.3)    ! dimensionless
    ...
    call g_tracer_add_param('phi_sldon_vmmdz',zoo(4)%phi_sldon, 0.2*0.3)     ! dimensionless
    ...
    call g_tracer_add_param('phi_sldop_vmmdz',zoo(4)%phi_sldop, 0.2*0.3)     ! dimensionless
    ...
    call g_tracer_add_param('kirr_hp',     cobalt%kirr_hp, 0.1)            ! W m-2         (mpoupon)
    ...
    call g_tracer_add_param('hp_phi_vis',  cobalt%hp_phi_vis, 0.90)        ! dimensionless (mpoupon)
...
!            implementation='duce',   &
    ...
    !
    !     Vertically migrating medium zooplankton N ! mpoupon
    !
    call g_tracer_add(tracer_list,package_name,&
         name       = 'nvmmdz',     &
         longname   = 'Vertically Migrating Medium Zooplankton Nitrogen (Biomass)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )
    ...
    call g_tracer_add(tracer_list,package_name,&
         name       = 'nvmmdz_gut',     &
         longname   = 'Vertically Migrating Medium Zooplankton Nitrogen (Gut)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )
    ...
    call g_tracer_add(tracer_list,package_name,&
         name       = 'pvmmdz_gut',     &
         longname   = 'Vertically Migrating Medium Zooplankton Phosphate (Gut)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )
    ...
    call g_tracer_add(tracer_list,package_name,&
         name       = 'fevmmdz_gut',     &
         longname   = 'Vertically Migrating Medium Zooplankton Iron (Gut)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )
    ...
    call g_tracer_add(tracer_list,package_name,&
         name       = 'sivmmdz_gut',     &
         longname   = 'Vertically Migrating Medium Zooplankton Silica (Gut)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )
    ...
    call g_tracer_add(tracer_list,package_name,&
         name       = 'nvmmdz_met',     &
         longname   = 'Vertically Migrating Medium Zooplankton Nitrogen (Metabolism)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )

    !
    !     Vertically migrating large zooplankton N ! mpoupon
    !
    call g_tracer_add(tracer_list,package_name,&
         name       = 'nvmlgz',     &
         longname   = 'Vertically Migrating Large Zooplankton Nitrogen (Biomass)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )
    ...
    call g_tracer_add(tracer_list,package_name,&
         name       = 'nvmlgz_gut',     &
         longname   = 'Vertically Migrating Large Zooplankton Nitrogen (Gut)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )
    ...
    call g_tracer_add(tracer_list,package_name,&
         name       = 'pvmlgz_gut',     &
         longname   = 'Vertically Migrating Large Zooplankton Phosphate (Gut)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )
    ...
    call g_tracer_add(tracer_list,package_name,&
         name       = 'fevmlgz_gut',     &
         longname   = 'Vertically Migrating Large Zooplankton Iron (Gut)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )
    ...
    call g_tracer_add(tracer_list,package_name,&
         name       = 'sivmlgz_gut',     &
         longname   = 'Vertically Migrating Large Zooplankton Silica (Gut)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )
     ...
     call g_tracer_add(tracer_list,package_name,&
         name       = 'nvmlgz_met',     &
         longname   = 'Vertically Migrating Large Zooplankton Nitrogen (Metabolism)', &
         units      = 'mol/kg',   &
         prog       = .true.,     &
         move_vertical = .true.   )
  ...
  subroutine generic_COBALT_update_from_source(tracer_list,Temp,Salt,rho_dzt,dzt,hblt_depth,&
    ...
    real, dimension(:,:,:), Allocatable :: z_remin_ramp, z_formig
...
!
    ...
    call g_tracer_get_values(tracer_list,'nvmmdz_gut'  ,'field',zoo(4)%f_gut_n(:,:,:)  ,isd,jsd,ntau=tau,positive=.true.)
    call g_tracer_get_values(tracer_list,'nvmlgz_gut'  ,'field',zoo(5)%f_gut_n(:,:,:)  ,isd,jsd,ntau=tau,positive=.true.)
    call g_tracer_get_values(tracer_list,'pvmmdz_gut'  ,'field',zoo(4)%f_gut_p(:,:,:)  ,isd,jsd,ntau=tau,positive=.true.)
    call g_tracer_get_values(tracer_list,'pvmlgz_gut'  ,'field',zoo(5)%f_gut_p(:,:,:)  ,isd,jsd,ntau=tau,positive=.true.)
    call g_tracer_get_values(tracer_list,'fevmmdz_gut' ,'field',zoo(4)%f_gut_fe(:,:,:) ,isd,jsd,ntau=tau,positive=.true.)
    call g_tracer_get_values(tracer_list,'fevmlgz_gut' ,'field',zoo(5)%f_gut_fe(:,:,:) ,isd,jsd,ntau=tau,positive=.true.)
    call g_tracer_get_values(tracer_list,'sivmmdz_gut' ,'field',zoo(4)%f_gut_si(:,:,:) ,isd,jsd,ntau=tau,positive=.true.)
    call g_tracer_get_values(tracer_list,'sivmlgz_gut' ,'field',zoo(5)%f_gut_si(:,:,:) ,isd,jsd,ntau=tau,positive=.true.)
    call g_tracer_get_values(tracer_list,'nvmmdz_met'  ,'field',zoo(4)%f_met_n(:,:,:)  ,isd,jsd,ntau=tau,positive=.true.)
    call g_tracer_get_values(tracer_list,'nvmlgz_met'  ,'field',zoo(5)%f_met_n(:,:,:)  ,isd,jsd,ntau=tau,positive=.true.)
    ...
    do k = 1, nk  ; do j = jsc, jec ; do i = isc, iec
       ...
       cobalt%jprod_nh4_conso(i,j,k) = 0.0              ! mpoupon
       cobalt%jprod_nh4_resp(i,j,k) = 0.0               ! mpoupon
       cobalt%jprod_nh4_passive(i,j,k) = 0.0            ! mpoupon
       cobalt%jprod_ndet_passive(i,j,k) = 0.0           ! mpoupon
       cobalt%jprod_don_passive(i,j,k) = 0.0            ! mpoupon
       cobalt%jprod_nh4_active_respiration(i,j,k) = 0.0 ! mpoupon
       cobalt%jprod_ndet_active_digestion(i,j,k) = 0.0  ! mpoupon
       cobalt%jprod_ndet_active_mortality(i,j,k) = 0.0  ! mpoupon
       cobalt%jprod_don_active_excretion(i,j,k) = 0.0   ! mpoupon
    enddo;  enddo ;  enddo !} i,j,k
!
!-----------------------------------------------------------------------------------
...
!
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec   !{
       ...
       cobalt%jprod_nh4_resp(i,j,k)    = cobalt%jprod_nh4_resp(i,j,k) - min(0.0,phyto(n)%mu(i,j,k)*phyto(n)%f_n(i,j,k))     ! mpoupon - budget
       cobalt%jprod_nh4_passive(i,j,k) = cobalt%jprod_nh4_passive(i,j,k) - min(0.0,phyto(n)%mu(i,j,k)*phyto(n)%f_n(i,j,k))  ! mpoupon - budget 
       do n = 2, NUM_PHYTO !{
...
!
    ...
    do k = 1, nk  ; do j = jsc, jec ; do i = isc, iec   !{
       ...
       cobalt%jprod_don_passive(i,j,k) = cobalt%jprod_don_passive(i,j,k) - max(bact(1)%jprod_n_het(i,j,k),0.0) ! mpoupon - budget
...
!
    ...
    !
    !  Calculate zooplankton and prey distributions for smart migration  
    !  (added by mpoupon)
    !
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec; !{
       ...
       ! Visual limitation (added by mpoupon - similar to Bianchi et al. 2013)
       cobalt%hp_vis_lim(i,j,k) =  (1 - cobalt%hp_phi_vis)  + cobalt%hp_phi_vis * cobalt%irr_inst(i,j,k)  &
                                    / (cobalt%irr_inst(i,j,k) +  cobalt%kirr_hp  *  cobalt%ki_hp / (cobalt%ki_hp + tot_prey_hp)) 

       hp_ingest_vec(6) = cobalt%hp_temp_lim(i,j,k)*cobalt%hp_o2lim(i,j,k)*cobalt%hp_vis_lim(i,j,k)*cobalt%imax_hp* &
       ...
       cobalt%hp_jingest_n_passive(i,j,k) = hp_ingest_vec(6) + hp_ingest_vec(7) ! mpoupon - budget
       cobalt%hp_jingest_n_active(i,j,k)  = hp_ingest_vec(8) + hp_ingest_vec(9) ! mpoupon - budget
    ...
    call g_tracer_get_pointer(tracer_list,'nvmmdz_gut','vmove',zoo(4)%p_vmove_gut)
    call g_tracer_get_pointer(tracer_list,'nvmlgz_gut','vmove',zoo(5)%p_vmove_gut)
    call g_tracer_get_pointer(tracer_list,'pvmmdz_gut','vmove',zoo(4)%p_vmove_gut_p)
    call g_tracer_get_pointer(tracer_list,'pvmlgz_gut','vmove',zoo(5)%p_vmove_gut_p)
    call g_tracer_get_pointer(tracer_list,'fevmmdz_gut','vmove',zoo(4)%p_vmove_gut_fe)
    call g_tracer_get_pointer(tracer_list,'fevmlgz_gut','vmove',zoo(5)%p_vmove_gut_fe)
    call g_tracer_get_pointer(tracer_list,'sivmmdz_gut','vmove',zoo(4)%p_vmove_gut_si)
    call g_tracer_get_pointer(tracer_list,'sivmlgz_gut','vmove',zoo(5)%p_vmove_gut_si)
    call g_tracer_get_pointer(tracer_list,'nvmmdz_met','vmove',zoo(4)%p_vmove_met)
    call g_tracer_get_pointer(tracer_list,'nvmlgz_met','vmove',zoo(5)%p_vmove_met)
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec; !{
       ...
       do n = 2, NUM_ZOO !{
           ...
           if ( cobalt%irr_inst(i,j,k) .lt. zoo(n)%dvm_I_thresh ) then
             ...
             ! Smart swimming outside mixing layer
             ! if ( hblt_depth(i,j) .lt. cobalt%zm(i,j,k) ) then 
              
               ! Medium migratory zooplankton (4)
               if ( n .eq. 4 ) then
                  if (vmmd_prey_norm_cum(i,j,k) .gt.  vmmd_norm_cum(i,j,k)) then
                     zoo(n)%p_vmove(i,j,k) = -swim ! Upward
                     zoo(n)%p_vmove_met(i,j,k) = -swim ! Upward  
                     zoo(n)%p_vmove_gut(i,j,k) = -swim ! Upward
                     zoo(n)%p_vmove_gut_p(i,j,k)  = -swim ! Upward
                     zoo(n)%p_vmove_gut_fe(i,j,k) = -swim ! Upward 
                     zoo(n)%p_vmove_gut_si(i,j,k) = -swim ! Upward   
                  else
                     zoo(n)%p_vmove(i,j,k) = swim  ! Downward
                     zoo(n)%p_vmove_met(i,j,k) = swim ! Downward  
                     zoo(n)%p_vmove_gut(i,j,k) = swim ! Downward     
                     zoo(n)%p_vmove_gut_p(i,j,k)  = swim ! Downward 
                     zoo(n)%p_vmove_gut_fe(i,j,k) = swim ! Downward
                     zoo(n)%p_vmove_gut_si(i,j,k) = swim ! Downward 
                  endif
             ...
             ! No swimming inside mixing layer
             !else
             !     zoo(n)%p_vmove(i,j,k) = 0
             !endif
           ...
           ! Downward swimming (during day)
               ...
               if ( n .eq. 4 ) then
                   ...
                   ! Not enough oxygen
                        ...
                        zoo(n)%p_vmove(i,j,k) = 0.0 ! No swimming
                        zoo(n)%p_vmove_met(i,j,k) = 0.0 ! No swimming
                        zoo(n)%p_vmove_gut(i,j,k) = 0.0 ! No swimming
                        zoo(n)%p_vmove_gut_p(i,j,k)  = 0.0 ! No swimming
                        zoo(n)%p_vmove_gut_fe(i,j,k) = 0.0 ! No swimming
                        zoo(n)%p_vmove_gut_si(i,j,k) = 0.0 ! No swimming
                   endif
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec   !{
       ...
       do m = 1,NUM_ZOO
           ...
           if ( m .eq. 4 .or. m .eq. 5) then
                 ...
                 zoo(m)%lim_nut_n_ingestion(i,j,k) = min(zoo(m)%jclear_gut_n(i,j,k), zoo(m)%jclear_gut_p(i,j,k)/zoo(m)%q_p_2_n) ! Move only to non migratory

                 zoo(m)%jprod_ndet(i,j,k)   = (1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_det   * zoo(m)%lim_nut_n_ingestion(i,j,k)
                 zoo(m)%jprod_pdet(i,j,k)   = (1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_det   * zoo(m)%lim_nut_n_ingestion(i,j,k)*zoo(m)%q_p_2_n
                 zoo(m)%jprod_sldon(i,j,k)  = (1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_sldon * zoo(m)%lim_nut_n_ingestion(i,j,k)
                 zoo(m)%jprod_ldon(i,j,k)   = (1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_ldon  * zoo(m)%lim_nut_n_ingestion(i,j,k)
                 zoo(m)%jprod_srdon(i,j,k)  = (1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_srdon * zoo(m)%lim_nut_n_ingestion(i,j,k)
                 zoo(m)%jprod_sldop(i,j,k)  = (1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_sldop * zoo(m)%lim_nut_n_ingestion(i,j,k)*zoo(m)%q_p_2_n
                 zoo(m)%jprod_ldop(i,j,k)   = (1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_ldop  * zoo(m)%lim_nut_n_ingestion(i,j,k)*zoo(m)%q_p_2_n
                 zoo(m)%jprod_srdop(i,j,k)  = (1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_srdop * zoo(m)%lim_nut_n_ingestion(i,j,k)*zoo(m)%q_p_2_n
          
                 zoo(m)%jprod_fedet(i,j,k)  = (1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_det * zoo(m)%jclear_gut_fe(i,j,k)
                 zoo(m)%jprod_sidet(i,j,k)  = (1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_det * zoo(m)%jclear_gut_si(i,j,k)

                 cobalt%jprod_don_active_excretion(i,j,k) = cobalt%jprod_don_active_excretion(i,j,k) + zoo(m)%jprod_sldon(i,j,k) &
                                                             + zoo(m)%jprod_ldon(i,j,k) + zoo(m)%jprod_srdon(i,j,k) ! mpoupon - budget
           ...
           ! Others
                 ...
                 cobalt%jprod_don_passive(i,j,k) = cobalt%jprod_don_passive(i,j,k) + zoo(m)%jprod_sldon(i,j,k) &
                                                   + zoo(m)%jprod_ldon(i,j,k) + zoo(m)%jprod_srdon(i,j,k) ! mpoupon - budget
       ...
       do m = 1,NUM_PHYTO
           ...
           cobalt%jprod_ndet_passive(i,j,k) = cobalt%jprod_ndet_passive(i,j,k) + phyto(m)%jaggloss_n(i,j,k) ! mpoupon - budget
       ...
       do m = 1,NUM_PHYTO
           ...
           cobalt%jprod_don_passive(i,j,k) = cobalt%jprod_don_passive(i,j,k)+ phyto(m)%jvirloss_n(i,j,k) + phyto(m)%jexuloss_n(i,j,k) ! mpoupon - budget
       ...
       cobalt%jprod_don_passive(i,j,k) = cobalt%jprod_don_passive(i,j,k) + bact(1)%jvirloss_n(i,j,k)  ! mpoupon - budget
       ...
       cobalt%jprod_srdon(i,j,k) = cobalt%jprod_srdon(i,j,k) - cobalt%lysis_phi_srdon* &
                                  min(bact(1)%jprod_n_het(i,j,k),0.0)
       cobalt%jprod_don_passive(i,j,k) = cobalt%jprod_don_passive(i,j,k) - min(bact(1)%jprod_n_het(i,j,k),0.0) ! mpoupon - budget
       ...
       do m = 1,NUM_ZOO
          ...
          if ( m .eq. 4 .or. m .eq. 5) then
             ...
             zoo(m)%jprod_met_n(i,j,k)   = zoo(m)%assim_eff(i,j,k) * zoo(m)%lim_nut_n_ingestion(i,j,k) 
             zoo(m)%jclear_met_n(i,j,k)  = zoo(m)%f_met_n(i,j,k)   * zoo(m)%k_clear_met

             zoo(m)%jmetabo_n(i,j,k) = basal_respiration * (1 + abs(zoo(m)%p_vmove(i,j,k)) / zoo(m)%swim_ref) + & 
                                       zoo(m)%phi_aresp * zoo(m)%jclear_met_n(i,j,k)
          ...
          ! Others
             ...
             zoo(m)%lim_nut_n_ingestion(i,j,k)  =  min(zoo(m)%jingest_n(i,j,k), zoo(m)%jingest_p(i,j,k)/zoo(m)%q_p_2_n)
             zoo(m)%jmetabo_n(i,j,k) =  basal_respiration +  (zoo(m)%phi_aresp*zoo(m)%lim_nut_n_ingestion(i,j,k))
             zoo(m)%jprod_n(i,j,k)   =  zoo(m)%assim_eff(i,j,k)*zoo(m)%lim_nut_n_ingestion(i,j,k) - zoo(m)%jmetabo_n(i,j,k)
             zoo(m)%jprod_nh4(i,j,k) =  zoo(m)%assim_eff(i,j,k)*zoo(m)%jingest_n(i,j,k) -  max(zoo(m)%jprod_n(i,j,k),0.0)
             zoo(m)%jprod_po4(i,j,k) =  zoo(m)%assim_eff(i,j,k)*zoo(m)%jingest_p(i,j,k) -  max(zoo(m)%jprod_n(i,j,k)*zoo(m)%q_p_2_n,0.0)
             zoo(m)%jprod_fed(i,j,k)  = zoo(m)%jingest_fe(i,j,k)   -  ((1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_det*zoo(m)%jingest_fe(i,j,k))
             zoo(m)%jprod_sio4(i,j,k) = zoo(m)%jingest_sio2(i,j,k) -  ((1.0-zoo(m)%assim_eff(i,j,k))*zoo(m)%phi_det*zoo(m)%jingest_sio2(i,j,k))
          endif
          ...
          if (zoo(m)%jprod_n(i,j,k) .lt. 0.0) then
             ...
             if ( m .eq. 4 .or. m .eq. 5) then
                   cobalt%jprod_ndet_active_mortality(i,j,k) = cobalt%jprod_ndet_active_mortality(i,j,k) - zoo(m)%jprod_n(i,j,k) ! mpoupon - budget  
             endif
          ...
          cobalt%jprod_nh4_resp(i,j,k) = cobalt%jprod_nh4_resp(i,j,k) + zoo(m)%jprod_nh4(i,j,k) ! mpoupon - budget
       ...
       cobalt%jprod_nh4_passive(i,j,k)  = cobalt%jprod_nh4_passive(i,j,k)  + zoo(1)%jprod_nh4(i,j,k)  + zoo(2)%jprod_nh4(i,j,k) + zoo(3)%jprod_nh4(i,j,k)   ! mpoupon - budget
       cobalt%jprod_nh4_active_respiration(i,j,k)   = cobalt%jprod_nh4_active_respiration(i,j,k)   + zoo(4)%jprod_nh4(i,j,k)  + zoo(5)%jprod_nh4(i,j,k)     ! mpoupon - budget
       cobalt%jprod_ndet_passive(i,j,k) = cobalt%jprod_ndet_passive(i,j,k) + zoo(1)%jprod_ndet(i,j,k) + zoo(2)%jprod_ndet(i,j,k) + zoo(3)%jprod_ndet(i,j,k) ! mpoupon - budget
       cobalt%jprod_ndet_active_digestion(i,j,k) = cobalt%jprod_ndet_active_digestion(i,j,k) + zoo(4)%jprod_ndet(i,j,k) &
                                                 + zoo(5)%jprod_ndet(i,j,k) - cobalt%jprod_ndet_active_mortality(i,j,k) ! mpoupon - budget
       ...
       cobalt%jprod_nh4_resp(i,j,k)     = cobalt%jprod_nh4_resp(i,j,k)    + (1.0-cobalt%hp_phi_det)*cobalt%hp_jingest_n(i,j,k)         ! mpoupon - budget
       cobalt%jprod_nh4_passive(i,j,k)  = cobalt%jprod_nh4_passive(i,j,k) + (1.0-cobalt%hp_phi_det)*cobalt%hp_jingest_n_passive(i,j,k) ! mpoupon - budget
       cobalt%jprod_nh4_active_respiration(i,j,k)   = cobalt%jprod_nh4_active_respiration(i,j,k)  + (1.0-cobalt%hp_phi_det)*cobalt%hp_jingest_n_active(i,j,k)  ! mpoupon - budget
       cobalt%jprod_ndet_passive(i,j,k) = cobalt%jprod_ndet_passive(i,j,k) + cobalt%hp_phi_det*cobalt%hp_jingest_n_passive(i,j,k)      ! mpoupon - budget
       cobalt%jprod_ndet_active_mortality(i,j,k)  = cobalt%jprod_ndet_active_mortality(i,j,k)  + cobalt%hp_phi_det*cobalt%hp_jingest_n_active(i,j,k)  ! mpoupon - budget
...
!
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec   !{
        cobalt%jprod_cadet_arag(i,j,k) = (zoo(2)%jzloss_n(i,j,k)*((1.0-zoo(3)%assim_eff(i,j,k))*zoo(3)%phi_det + (1.0-zoo(5)%assim_eff(i,j,k))*zoo(5)%phi_det)/2.0 + &
                       zoo(4)%jzloss_n(i,j,k)*((1.0-zoo(3)%assim_eff(i,j,k))*zoo(3)%phi_det + (1.0-zoo(5)%assim_eff(i,j,k))*zoo(5)%phi_det)/2.0 + &
        ...
        cobalt%jprod_cadet_calc(i,j,k) = (zoo(1)%jzloss_n(i,j,k)*((1.0-zoo(2)%assim_eff(i,j,k))*zoo(2)%phi_det + (1.0-zoo(4)%assim_eff(i,j,k))*zoo(4)%phi_det)/2.0 + &
                       phyto(SMALL)%jzloss_n(i,j,k)*(1.0-zoo(1)%assim_eff(i,j,k))*zoo(1)%phi_det + &
                       phyto(LARGE)%jzloss_n(i,j,k)*((1.0-zoo(3)%assim_eff(i,j,k))*zoo(3)%phi_det + (1.0-zoo(5)%assim_eff(i,j,k))*zoo(5)%phi_det)/2.0 + &
...
!
    ...
    !(mpoupon) migratory zooplankton physiology
...
!
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
       ...
       cobalt%jdic(i,j,k) =(cobalt%c_2_n * (cobalt%jprod_nh4(i,j,k) - &
        ...
        cobalt%jprod_nh4_conso(i,j,k) = - phyto(DIAZO)%juptake_no3(i,j,k) - phyto(LARGE)%juptake_no3(i,j,k) - phyto(SMALL)%juptake_no3(i,j,k) & ! mpoupon
                                        - phyto(DIAZO)%juptake_nh4(i,j,k) - phyto(LARGE)%juptake_nh4(i,j,k) - phyto(SMALL)%juptake_nh4(i,j,k) &
                                        - phyto(DIAZO)%juptake_n2(i,j,k)  - bact(1)%jprod_n_amx(i,j,k)      - bact(1)%jprod_n_nitrif(i,j,k)               
...
!
    ...
    cobalt%tot_layer_int_n(:,:,:) = (cobalt%p_no3(:,:,:,tau) + &
         ...
         cobalt%p_nvmmdz_met(:,:,:,tau) + cobalt%p_nvmlgz_met(:,:,:,tau)) * rho_dzt(:,:,:)
    ...
    cobalt%tot_layer_int_p(:,:,:) = (cobalt%p_po4(:,:,:,tau) + &
         ...
         zoo(4)%q_p_2_n*cobalt%p_nvmmdz_gut(:,:,:,tau) + zoo(5)%q_p_2_n*cobalt%p_nvmlgz_gut(:,:,:,tau) + &
    ...
    cobalt%tot_layer_int_poc(:,:,:) = (cobalt%p_ndi(:,:,:,tau) + cobalt%p_nlg(:,:,:,tau) + cobalt%p_nsm(:,:,:,tau) + &
         ...
         cobalt%p_nlgz(:,:,:,tau) + cobalt%p_nvmmdz(:,:,:,tau) + cobalt%p_nvmlgz(:,:,:,tau) + cobalt%p_nvmmdz_gut(:,:,:,tau) + &
         cobalt%p_nvmlgz_gut(:,:,:,tau) + cobalt%p_nvmmdz_met(:,:,:,tau) + cobalt%p_nvmlgz_met(:,:,:,tau))*cobalt%c_2_n*rho_dzt(:,:,:)
    
    cobalt%tot_layer_int_pic(:,:,:) = (cobalt%p_cadet_arag(:,:,:,tau) + cobalt%p_cadet_calc(:,:,:,tau))*rho_dzt(:,:,:) ! mpoupon 
    ...
    do j = jsc, jec ; do i = isc, iec !{
       ...
       ! mpoupon - plankton dynamics
       do n = 1, NUM_ZOO  !{
           zoo(n)%wc_vert_int_jingest_n(i,j) = 0.0
           zoo(n)%wc_vert_int_jmetabo_n(i,j) = 0.0
           zoo(n)%wc_vert_int_jmetabo_bas_n(i,j) = 0.0
           zoo(n)%wc_vert_int_jmetabo_dig_n(i,j) = 0.0
           zoo(n)%wc_vert_int_jmetabo_swm_n(i,j) = 0.0
           zoo(n)%wc_vert_int_jprod_n(i,j) = 0.0
           zoo(n)%wc_vert_int_jprodpos_n(i,j) = 0.0
           zoo(n)%wc_vert_int_jmort_n(i,j) = 0.0
           zoo(n)%wc_vert_int_jzloss_n(i,j) = 0.0
           zoo(n)%wc_vert_int_jhploss_n(i,j) = 0.0
           zoo(n)%wc_vert_int_jzoo_n(i,j) = 0.0
           zoo(n)%wc_vert_int_f_n(i,j) = 0.0
           zoo(n)%z_mig(i,j) = 0.0
       enddo   !} n

       do n = 1, NUM_PHYTO  !{
           phyto(n)%wc_vert_int_maxgrowth_lightlim_n(i,j) = 0.0
           phyto(n)%wc_vert_int_maxgrowth_nutlim_n(i,j)   = 0.0
           phyto(n)%wc_vert_int_maxgrowth_n(i,j)          = 0.0
           phyto(n)%wc_vert_int_jprod_n(i,j)              = 0.0
           phyto(n)%wc_vert_int_jresp_n(i,j)              = 0.0
           phyto(n)%wc_vert_int_jvirloss_n(i,j)           = 0.0
           phyto(n)%wc_vert_int_jaggloss_n(i,j)           = 0.0
           phyto(n)%wc_vert_int_jexuloss_n(i,j)           = 0.0
           phyto(n)%wc_vert_int_jzloss_n(i,j)             = 0.0
           phyto(n)%wc_vert_int_jphyto_n(i,j)             = 0.0
           phyto(n)%wc_vert_int_f_n(i,j)                  = 0.0
       enddo   !} n
       ! mpoupon - plankton dynamics
    enddo; enddo !} i,j
    ...
    ! Calculate the depth for scaling of remineralization near the surface
    allocate(z_formig(isd:ied,jsd:jed,1:nk)); z_formig = 0.0
    z_formig(:,:,1) = dzt(:,:,1)
    do k = 2,nk !{
      z_formig(:,:,k) = z_formig(:,:,k-1) + dzt(:,:,k)
    enddo !}k
    ...
    do j = jsc, jec ; do i = isc, iec ; do k = 1, nk  !{
          ...
          ! mpoupon - plankton dynamics
          do n = 1, NUM_PHYTO  !{
    phyto(n)%wc_vert_int_maxgrowth_lightlim_n(i,j) = phyto(n)%wc_vert_int_maxgrowth_lightlim_n(i,j) + max(phyto(n)%liebig_lim(i,j,k)*phyto(n)%P_C_max*cobalt%expkT(i,j,k),epsln)/(1.0 + cobalt%zeta)*phyto(n)%f_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
    phyto(n)%wc_vert_int_maxgrowth_nutlim_n(i,j)   = phyto(n)%wc_vert_int_maxgrowth_nutlim_n(i,j) + max(phyto(n)%irrlim(i,j,k)*phyto(n)%P_C_max*cobalt%expkT(i,j,k),epsln)/(1.0 + cobalt%zeta)*phyto(n)%f_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
    phyto(n)%wc_vert_int_maxgrowth_n(i,j)          = phyto(n)%wc_vert_int_maxgrowth_n(i,j) + max(phyto(n)%P_C_max*cobalt%expkT(i,j,k),epsln)/(1.0 + cobalt%zeta)*phyto(n)%f_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
    phyto(n)%wc_vert_int_jprod_n(i,j)              = phyto(n)%wc_vert_int_jprod_n(i,j) + phyto(n)%jprod_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
    phyto(n)%wc_vert_int_jresp_n(i,j)              = phyto(n)%wc_vert_int_jresp_n(i,j) - cobalt%expkT(i,j,k)*phyto(n)%bresp*phyto(n)%f_n(i,j,k)/(cobalt%refuge_conc + phyto(n)%f_n(i,j,k))*phyto(n)%f_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
    phyto(n)%wc_vert_int_jvirloss_n(i,j)           = phyto(n)%wc_vert_int_jvirloss_n(i,j) + phyto(n)%jvirloss_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
    phyto(n)%wc_vert_int_jaggloss_n(i,j)           = phyto(n)%wc_vert_int_jaggloss_n(i,j) + phyto(n)%jaggloss_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
    phyto(n)%wc_vert_int_jexuloss_n(i,j)           = phyto(n)%wc_vert_int_jexuloss_n(i,j) + phyto(n)%jexuloss_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
    phyto(n)%wc_vert_int_jzloss_n(i,j)             = phyto(n)%wc_vert_int_jzloss_n(i,j) + phyto(n)%jzloss_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
    phyto(n)%wc_vert_int_f_n(i,j)                  = phyto(n)%wc_vert_int_f_n(i,j) + phyto(n)%f_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
enddo   !} n


phyto(SMALL)%wc_vert_int_jphyto_n(i,j) = phyto(SMALL)%wc_vert_int_jphyto_n(i,j)  + cobalt%jnsm(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
phyto(LARGE)%wc_vert_int_jphyto_n(i,j) = phyto(LARGE)%wc_vert_int_jphyto_n(i,j)  + cobalt%jnlg(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
phyto(DIAZO)%wc_vert_int_jphyto_n(i,j) = phyto(DIAZO)%wc_vert_int_jphyto_n(i,j)  + cobalt%jndi(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)


          do n = 1, NUM_ZOO  !{
             zoo(n)%wc_vert_int_jingest_n(i,j) = zoo(n)%wc_vert_int_jingest_n(i,j) + zoo(n)%lim_nut_n_ingestion(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
             zoo(n)%wc_vert_int_jmetabo_n(i,j) = zoo(n)%wc_vert_int_jmetabo_n(i,j) + zoo(n)%jmetabo_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
             basal_respiration = zoo(n)%f_n(i,j,k)/(cobalt%refuge_conc + zoo(n)%f_n(i,j,k))* &
                              zoo(n)%temp_lim(i,j,k)*zoo(n)%bresp*zoo(n)%f_n(i,j,k)
             if ( n .eq. 4 .or. n .eq. 5) then
                zoo(n)%wc_vert_int_jmetabo_bas_n(i,j) = zoo(n)%wc_vert_int_jmetabo_bas_n(i,j) + basal_respiration*rho_dzt(i,j,k)*grid_tmask(i,j,k) 
                zoo(n)%wc_vert_int_jmetabo_dig_n(i,j) = zoo(n)%wc_vert_int_jmetabo_dig_n(i,j) + zoo(n)%phi_aresp * zoo(n)%jclear_met_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
                zoo(n)%wc_vert_int_jmetabo_swm_n(i,j) = zoo(n)%wc_vert_int_jmetabo_swm_n(i,j) + basal_respiration * abs(zoo(n)%p_vmove(i,j,k)) / zoo(n)%swim_ref *rho_dzt(i,j,k)*grid_tmask(i,j,k)
             else
                zoo(n)%wc_vert_int_jmetabo_bas_n(i,j) = zoo(n)%wc_vert_int_jmetabo_bas_n(i,j) + basal_respiration*rho_dzt(i,j,k)*grid_tmask(i,j,k)
                zoo(n)%wc_vert_int_jmetabo_dig_n(i,j) = zoo(n)%wc_vert_int_jmetabo_dig_n(i,j) + (zoo(n)%phi_aresp*zoo(n)%lim_nut_n_ingestion(i,j,k))*rho_dzt(i,j,k)*grid_tmask(i,j,k) 
             endif
             zoo(n)%wc_vert_int_jprod_n(i,j)   = zoo(n)%wc_vert_int_jprod_n(i,j)   + zoo(n)%jprod_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
             zoo(n)%wc_vert_int_jprodpos_n(i,j)   = zoo(n)%wc_vert_int_jprodpos_n(i,j) + max(zoo(n)%jprod_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k), 0.0)
             zoo(n)%wc_vert_int_jmort_n(i,j)   = zoo(n)%wc_vert_int_jmort_n(i,j)   - min(zoo(n)%jprod_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k), 0.0)
             zoo(n)%wc_vert_int_jzloss_n(i,j)  = zoo(n)%wc_vert_int_jzloss_n(i,j)  + zoo(n)%jzloss_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
             zoo(n)%wc_vert_int_jhploss_n(i,j) = zoo(n)%wc_vert_int_jhploss_n(i,j) + zoo(n)%jhploss_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
             zoo(n)%wc_vert_int_f_n(i,j)       = zoo(n)%wc_vert_int_f_n(i,j)       + zoo(n)%f_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
             zoo(n)%z_mig(i,j)       = zoo(n)%z_mig(i,j) + z_formig(i,j,k)*zoo(n)%f_n(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k) ! mpoupon
          enddo   !} n

          zoo(1)%wc_vert_int_jzoo_n(i,j) = zoo(1)%wc_vert_int_jzoo_n(i,j) + cobalt%jnsmz(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
          zoo(2)%wc_vert_int_jzoo_n(i,j) = zoo(2)%wc_vert_int_jzoo_n(i,j) + cobalt%jnmdz(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
          zoo(3)%wc_vert_int_jzoo_n(i,j) = zoo(3)%wc_vert_int_jzoo_n(i,j) + cobalt%jnlgz(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
          zoo(4)%wc_vert_int_jzoo_n(i,j) = zoo(4)%wc_vert_int_jzoo_n(i,j) + cobalt%jnvmmdz(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
          zoo(5)%wc_vert_int_jzoo_n(i,j) = zoo(5)%wc_vert_int_jzoo_n(i,j) + cobalt%jnvmlgz(i,j,k)*rho_dzt(i,j,k)*grid_tmask(i,j,k)
          ! mpoupon - plankton dynamics 
    enddo; enddo; enddo  !} i,j,k

    do j = jsc, jec ; do i = isc, iec ; do n = 1, NUM_ZOO !{
          zoo(n)%z_mig(i,j)   =  zoo(n)%z_mig(i,j) /  zoo(n)%wc_vert_int_f_n(i,j)
    enddo; enddo; enddo  !} n,i,j
...
!---------------------------------------------------------------------
!mpoupon
if (cobalt%id_rho_dzt .gt. 0)          &
        used = g_send_data(cobalt%id_rho_dzt, rho_dzt,           &
        model_time, rmask = grid_tmask,&
        is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
!mpoupon
...
! Send phytoplankton diagnostic data
    ...
    do n= 1, NUM_ZOO
       ...
       if (zoo(n)%id_jingest_n .gt. 0)          &
            ...
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
       !mpoupon
       if (zoo(n)%id_jingest_n_lim .gt. 0)          &
            used = g_send_data(zoo(n)%id_jingest_n_lim, zoo(n)%lim_nut_n_ingestion*rho_dzt,      &
            model_time, rmask = grid_tmask,&
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
       ! mpoupon
      ...
      if (zoo(n)%id_jprod_srdon .gt. 0)          &
       ...
       ! MPOUPON NOTE: ADD DIAGNOSTIC jprod and jclear gut and met here!
       !
       if (zoo(n)%id_jmetabo_n .gt. 0)          &
            used = g_send_data(zoo(n)%id_jmetabo_n, zoo(n)%jmetabo_n * rho_dzt,        &
            model_time, rmask = grid_tmask,&
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
    enddo
    !
   ...
   ! Biological Pump diag - mpoupon
   if (cobalt%id_jprod_nh4_conso .gt. 0)          &
        used = g_send_data(cobalt%id_jprod_nh4_conso, cobalt%jprod_nh4_conso*rho_dzt, &
        model_time, rmask = grid_tmask,&
        is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
   if (cobalt%id_jprod_nh4_resp .gt. 0)          &
        used = g_send_data(cobalt%id_jprod_nh4_resp, cobalt%jprod_nh4_resp*rho_dzt, &
        model_time, rmask = grid_tmask,&
        is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
   if (cobalt%id_jprod_nh4_passive .gt. 0)          &
        used = g_send_data(cobalt%id_jprod_nh4_passive, cobalt%jprod_nh4_passive*rho_dzt, &
        model_time, rmask = grid_tmask,&
        is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
   if (cobalt%id_jprod_ndet_passive .gt. 0)          &
        used = g_send_data(cobalt%id_jprod_ndet_passive, cobalt%jprod_ndet_passive*rho_dzt, &
        model_time, rmask = grid_tmask,&
        is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
   if (cobalt%id_jprod_don_passive .gt. 0)          &
        used = g_send_data(cobalt%id_jprod_don_passive, cobalt%jprod_don_passive*rho_dzt, &
        model_time, rmask = grid_tmask,&
        is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
   if (cobalt%id_jprod_nh4_active_respiration .gt. 0)          &
        used = g_send_data(cobalt%id_jprod_nh4_active_respiration, cobalt%jprod_nh4_active_respiration*rho_dzt, &
        model_time, rmask = grid_tmask,&
        is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
   if (cobalt%id_jprod_ndet_active_digestion .gt. 0)          &
        used = g_send_data(cobalt%id_jprod_ndet_active_digestion, cobalt%jprod_ndet_active_digestion*rho_dzt, &
        model_time, rmask = grid_tmask,&
        is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
   if (cobalt%id_jprod_ndet_active_mortality .gt. 0)          &
        used = g_send_data(cobalt%id_jprod_ndet_active_mortality, cobalt%jprod_ndet_active_mortality*rho_dzt, &
        model_time, rmask = grid_tmask,&
        is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
   if (cobalt%id_jprod_don_active_excretion .gt. 0)          &
        used = g_send_data(cobalt%id_jprod_don_active_excretion, cobalt%jprod_don_active_excretion*rho_dzt, &
        model_time, rmask = grid_tmask,&
        is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
   ! end biological pump diag - mpoupon
    ...
    if (cobalt%id_hp_temp_lim .gt. 0)            &
         ...
         model_time, rmask = grid_tmask,&
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
    if (cobalt%id_hp_vis_lim .gt. 0)            &
         used = g_send_data(cobalt%id_hp_vis_lim, cobalt%hp_vis_lim,         &
         model_time, rmask = grid_tmask,&
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)   
    if (cobalt%id_irr_inst .gt. 0)           &
         ...
         model_time, rmask = grid_tmask(:,:,:),&
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
    if (cobalt%id_irr_inst_cw .gt. 0)           &
         used = g_send_data(cobalt%id_irr_inst_cw,   cobalt%irr_inst_cw,           &
         model_time, rmask = grid_tmask(:,:,:),&
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
    ...
    if (cobalt%id_tot_layer_int_c .gt. 0)  &
         ...
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
    ! mpoupon
    if (cobalt%id_tot_layer_int_doc .gt. 0)  &
         used = g_send_data(cobalt%id_tot_layer_int_doc, cobalt%tot_layer_int_doc,&
         model_time, rmask = grid_tmask,&
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
    if (cobalt%id_tot_layer_int_poc .gt. 0)  &
         used = g_send_data(cobalt%id_tot_layer_int_poc, cobalt%tot_layer_int_poc,&
         model_time, rmask = grid_tmask,&
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
    if (cobalt%id_tot_layer_int_dic .gt. 0)  &
         used = g_send_data(cobalt%id_tot_layer_int_dic, cobalt%tot_layer_int_dic,&
         model_time, rmask = grid_tmask,&
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
    if (cobalt%id_tot_layer_int_pic .gt. 0)  &
         used = g_send_data(cobalt%id_tot_layer_int_pic, cobalt%tot_layer_int_pic,&
         model_time, rmask = grid_tmask,&
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
    ! mpoupon
   ...
   if (cobalt%id_btm_co3_sol_calc .gt. 0)            &
    ...
    do n= 1, NUM_PHYTO  !{
      ...
      if (phyto(n)%id_f_n_100 .gt. 0)     &
          ...
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      ! mpoupon - plankton dynamics 
      if (phyto(n)%id_wc_vert_int_maxgrowth_lightlim_n .gt. 0)     &
          used = g_send_data(phyto(n)%id_wc_vert_int_maxgrowth_lightlim_n, phyto(n)%wc_vert_int_maxgrowth_lightlim_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)

      if (phyto(n)%id_wc_vert_int_maxgrowth_nutlim_n .gt. 0)     &
          used = g_send_data(phyto(n)%id_wc_vert_int_maxgrowth_nutlim_n, phyto(n)%wc_vert_int_maxgrowth_nutlim_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)

      if (phyto(n)%id_wc_vert_int_maxgrowth_n .gt. 0)     &
          used = g_send_data(phyto(n)%id_wc_vert_int_maxgrowth_n, phyto(n)%wc_vert_int_maxgrowth_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)

      if (phyto(n)%id_wc_vert_int_jprod_n .gt. 0)     &
          used = g_send_data(phyto(n)%id_wc_vert_int_jprod_n, phyto(n)%wc_vert_int_jprod_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)

      if (phyto(n)%id_wc_vert_int_jresp_n .gt. 0)     &
          used = g_send_data(phyto(n)%id_wc_vert_int_jresp_n, phyto(n)%wc_vert_int_jresp_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)

      if (phyto(n)%id_wc_vert_int_jvirloss_n .gt. 0)     &
          used = g_send_data(phyto(n)%id_wc_vert_int_jvirloss_n, phyto(n)%wc_vert_int_jvirloss_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)

      if (phyto(n)%id_wc_vert_int_jaggloss_n .gt. 0)     &
          used = g_send_data(phyto(n)%id_wc_vert_int_jaggloss_n, phyto(n)%wc_vert_int_jaggloss_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)

      if (phyto(n)%id_wc_vert_int_jexuloss_n .gt. 0)     &
          used = g_send_data(phyto(n)%id_wc_vert_int_jexuloss_n, phyto(n)%wc_vert_int_jexuloss_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)

      if (phyto(n)%id_wc_vert_int_jzloss_n .gt. 0)     &
          used = g_send_data(phyto(n)%id_wc_vert_int_jzloss_n, phyto(n)%wc_vert_int_jzloss_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)

      if (phyto(n)%id_wc_vert_int_jphyto_n .gt. 0)     &
          used = g_send_data(phyto(n)%id_wc_vert_int_jphyto_n, phyto(n)%wc_vert_int_jphyto_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)

      if (phyto(n)%id_wc_vert_int_f_n .gt. 0)     &
          used = g_send_data(phyto(n)%id_wc_vert_int_f_n, phyto(n)%wc_vert_int_f_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
    ! mpoupon - plankton dynamics
    enddo !} n
   ...
   if (phyto(LARGE)%id_jaggloss_n_100 .gt. 0)     &
     ...
     do n= 1, NUM_ZOO  !{
       ...
       if (zoo(n)%id_jprod_ndet_100 .gt. 0)     &
          ...
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       ! mpoupon - plankton dynamics
       if (zoo(n)%id_wc_vert_int_jingest_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_jingest_n, zoo(n)%wc_vert_int_jingest_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_wc_vert_int_jmetabo_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_jmetabo_n, zoo(n)%wc_vert_int_jmetabo_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_wc_vert_int_jmetabo_bas_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_jmetabo_bas_n, zoo(n)%wc_vert_int_jmetabo_bas_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_wc_vert_int_jmetabo_dig_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_jmetabo_dig_n, zoo(n)%wc_vert_int_jmetabo_dig_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_wc_vert_int_jmetabo_swm_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_jmetabo_swm_n, zoo(n)%wc_vert_int_jmetabo_swm_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_wc_vert_int_jprod_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_jprod_n, zoo(n)%wc_vert_int_jprod_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_wc_vert_int_jprodpos_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_jprodpos_n, zoo(n)%wc_vert_int_jprodpos_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_wc_vert_int_jmort_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_jmort_n, zoo(n)%wc_vert_int_jmort_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_wc_vert_int_jzloss_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_jzloss_n, zoo(n)%wc_vert_int_jzloss_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_wc_vert_int_jhploss_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_jhploss_n, zoo(n)%wc_vert_int_jhploss_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_wc_vert_int_jzoo_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_jzoo_n, zoo(n)%wc_vert_int_jzoo_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_wc_vert_int_f_n .gt. 0)     &
          used = g_send_data(zoo(n)%id_wc_vert_int_f_n, zoo(n)%wc_vert_int_f_n,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       if (zoo(n)%id_z_mig .gt. 0)     &
          used = g_send_data(zoo(n)%id_z_mig, zoo(n)%z_mig,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       ! mpoupon - plankton dynamics
     enddo !} n
...
!liao
    ...
    if (cobalt%id_jnh4 .gt. 0)              &
     ...
     if (cobalt%id_jndet .gt. 0)              &
          ...
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
!mpoupon
     if (cobalt%id_jndetc .gt. 0)              &
          used = g_send_data(cobalt%id_jndetc, cobalt%jndet,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jsldonc .gt. 0)              &
          used = g_send_data(cobalt%id_jsldonc, cobalt%jsldon,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jldonc .gt. 0)              &
          used = g_send_data(cobalt%id_jldonc, cobalt%jldon,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jsrdonc .gt. 0)              &
          used = g_send_data(cobalt%id_jsrdonc, cobalt%jsrdon,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jndic .gt. 0)              &
          used = g_send_data(cobalt%id_jndic, cobalt%jndi,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jnlgc .gt. 0)              &
          used = g_send_data(cobalt%id_jnlgc, cobalt%jnlg,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jnsmc .gt. 0)              &
          used = g_send_data(cobalt%id_jnsmc, cobalt%jnsm,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jnbactc .gt. 0)              &
          used = g_send_data(cobalt%id_jnbactc, cobalt%jnbact,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jnlgzc .gt. 0)              &
          used = g_send_data(cobalt%id_jnlgzc, cobalt%jnlgz,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jnmdzc .gt. 0)              &
          used = g_send_data(cobalt%id_jnmdzc, cobalt%jnmdz,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jnvmlgzc .gt. 0)              &
          used = g_send_data(cobalt%id_jnvmlgzc, cobalt%jnvmlgz,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jnvmmdzc .gt. 0)              &
          used = g_send_data(cobalt%id_jnvmmdzc, cobalt%jnvmmdz,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jnsmzc .gt. 0)              &
          used = g_send_data(cobalt%id_jnsmzc, cobalt%jnsmz,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jcadet_aragc .gt. 0)              &
          used = g_send_data(cobalt%id_jcadet_aragc, cobalt%jcadet_arag,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

     if (cobalt%id_jcadet_calcc .gt. 0)              &
          used = g_send_data(cobalt%id_jcadet_calcc, cobalt%jcadet_calc,       &
          model_time, rmask = grid_tmask,&
          is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
...
!
  ...
  subroutine user_allocate_arrays
    ...
    do n = 1, NUM_ZOO
       ...
       allocate(zoo(n)%jingest_n_lim(isd:ied,jsd:jed,nk)) ; zoo(n)%jingest_n_lim  = 0.0 ! mpoupon
   ...
   enddo
    ...
    allocate(cobalt%hp_jingest_n_passive(isd:ied,jsd:jed,nk))    ; cobalt%hp_jingest_n_passive   = 0.0 ! mpoupon - budget
    allocate(cobalt%hp_jingest_n_active(isd:ied,jsd:jed,nk))     ; cobalt%hp_jingest_n_active    = 0.0 ! mpoupon - budget
    ...
    allocate(cobalt%jnvmmdz_gut(isd:ied, jsd:jed, 1:nk))  ; cobalt%jnvmmdz_gut=0.0 ! mpoupon
    allocate(cobalt%jnvmlgz_gut(isd:ied, jsd:jed, 1:nk))  ; cobalt%jnvmlgz_gut=0.0 ! mpoupon
    allocate(cobalt%jpvmmdz_gut(isd:ied, jsd:jed, 1:nk))  ; cobalt%jpvmmdz_gut=0.0 ! mpoupon
    allocate(cobalt%jpvmlgz_gut(isd:ied, jsd:jed, 1:nk))  ; cobalt%jpvmlgz_gut=0.0 ! mpoupon
    allocate(cobalt%jfevmmdz_gut(isd:ied, jsd:jed, 1:nk)) ; cobalt%jfevmmdz_gut=0.0 ! mpoupon
    allocate(cobalt%jfevmlgz_gut(isd:ied, jsd:jed, 1:nk)) ; cobalt%jfevmlgz_gut=0.0 ! mpoupon
    allocate(cobalt%jsivmmdz_gut(isd:ied, jsd:jed, 1:nk)) ; cobalt%jsivmmdz_gut=0.0 ! mpoupon
    allocate(cobalt%jsivmlgz_gut(isd:ied, jsd:jed, 1:nk)) ; cobalt%jsivmlgz_gut=0.0 ! mpoupon
    allocate(cobalt%jnvmmdz_met(isd:ied, jsd:jed, 1:nk))  ; cobalt%jnvmmdz_met=0.0 ! mpoupon
    allocate(cobalt%jnvmlgz_met(isd:ied, jsd:jed, 1:nk))  ; cobalt%jnvmlgz_met=0.0 ! mpoupon
    ...
    allocate(cobalt%jprod_nh4_conso(isd:ied, jsd:jed, 1:nk))      ; cobalt%jprod_nh4_conso=0.0    ! mpoupon
    allocate(cobalt%jprod_nh4_resp(isd:ied, jsd:jed, 1:nk))       ; cobalt%jprod_nh4_resp=0.0     ! mpoupon
    allocate(cobalt%jprod_nh4_passive(isd:ied, jsd:jed, 1:nk))    ; cobalt%jprod_nh4_passive=0.0  ! mpoupon
    allocate(cobalt%jprod_ndet_passive(isd:ied, jsd:jed, 1:nk))   ; cobalt%jprod_ndet_passive=0.0 ! mpoupon
    allocate(cobalt%jprod_don_passive(isd:ied, jsd:jed, 1:nk))    ; cobalt%jprod_don_passive=0.0  ! mpoupon
    allocate(cobalt%jprod_nh4_active_respiration(isd:ied, jsd:jed, 1:nk))  ; cobalt%jprod_nh4_active_respiration=0.0 ! mpoupon
    allocate(cobalt%jprod_ndet_active_digestion(isd:ied, jsd:jed, 1:nk))   ; cobalt%jprod_ndet_active_digestion=0.0  ! mpoupon
    allocate(cobalt%jprod_ndet_active_mortality(isd:ied, jsd:jed, 1:nk))   ; cobalt%jprod_ndet_active_mortality=0.0  ! mpoupon
    allocate(cobalt%jprod_don_active_excretion(isd:ied, jsd:jed, 1:nk))    ; cobalt%jprod_don_active_excretion=0.0   ! mpoupon
    ...
    allocate(cobalt%irr_inst_cw(isd:ied, jsd:jed, 1:nk))  ; cobalt%irr_inst_cw=0.0 ! mpoupon
    ...
    allocate(cobalt%tot_layer_int_doc(isd:ied, jsd:jed,1:nk))  ; cobalt%tot_layer_int_doc=0.0 ! mpoupon
    allocate(cobalt%tot_layer_int_poc(isd:ied, jsd:jed,1:nk))  ; cobalt%tot_layer_int_poc=0.0 ! mpoupon
    allocate(cobalt%tot_layer_int_dic(isd:ied, jsd:jed,1:nk))  ; cobalt%tot_layer_int_dic=0.0 ! mpoupon
    allocate(cobalt%tot_layer_int_pic(isd:ied, jsd:jed,1:nk))  ; cobalt%tot_layer_int_pic=0.0 ! mpoupon
...
!==============================================================================================================
    ...
    do n = 1, NUM_PHYTO
       ...
       ! mpoupon - plankton dynamics
       allocate(phyto(n)%wc_vert_int_maxgrowth_lightlim_n(isd:ied,jsd:jed)) ; phyto(n)%wc_vert_int_maxgrowth_lightlim_n  = 0.0
       allocate(phyto(n)%wc_vert_int_maxgrowth_nutlim_n(isd:ied,jsd:jed))   ; phyto(n)%wc_vert_int_maxgrowth_nutlim_n    = 0.0
       allocate(phyto(n)%wc_vert_int_maxgrowth_n(isd:ied,jsd:jed)) ; phyto(n)%wc_vert_int_maxgrowth_n    = 0.0
       allocate(phyto(n)%wc_vert_int_jprod_n(isd:ied,jsd:jed))     ; phyto(n)%wc_vert_int_jprod_n    = 0.0
       allocate(phyto(n)%wc_vert_int_jresp_n(isd:ied,jsd:jed))     ; phyto(n)%wc_vert_int_jresp_n    = 0.0
       allocate(phyto(n)%wc_vert_int_jvirloss_n(isd:ied,jsd:jed))  ; phyto(n)%wc_vert_int_jvirloss_n    = 0.0
       allocate(phyto(n)%wc_vert_int_jaggloss_n(isd:ied,jsd:jed))  ; phyto(n)%wc_vert_int_jaggloss_n    = 0.0
       allocate(phyto(n)%wc_vert_int_jexuloss_n(isd:ied,jsd:jed))  ; phyto(n)%wc_vert_int_jexuloss_n    = 0.0
       allocate(phyto(n)%wc_vert_int_jzloss_n(isd:ied,jsd:jed))    ; phyto(n)%wc_vert_int_jzloss_n    = 0.0
       allocate(phyto(n)%wc_vert_int_jphyto_n(isd:ied,jsd:jed))    ; phyto(n)%wc_vert_int_jphyto_n    = 0.0
       allocate(phyto(n)%wc_vert_int_f_n(isd:ied,jsd:jed))         ; phyto(n)%wc_vert_int_f_n    = 0.0
    enddo
    ...
    do n = 1, NUM_ZOO
       ...
       ! mpoupon - plankton dynamics
       allocate(zoo(n)%wc_vert_int_jingest_n(isd:ied,jsd:jed)) ; zoo(n)%wc_vert_int_jingest_n    = 0.0
       allocate(zoo(n)%wc_vert_int_jmetabo_n(isd:ied,jsd:jed)) ; zoo(n)%wc_vert_int_jmetabo_n    = 0.0
       allocate(zoo(n)%wc_vert_int_jmetabo_bas_n(isd:ied,jsd:jed)) ; zoo(n)%wc_vert_int_jmetabo_bas_n    = 0.0
       allocate(zoo(n)%wc_vert_int_jmetabo_dig_n(isd:ied,jsd:jed)) ; zoo(n)%wc_vert_int_jmetabo_dig_n    = 0.0
       allocate(zoo(n)%wc_vert_int_jmetabo_swm_n(isd:ied,jsd:jed)) ; zoo(n)%wc_vert_int_jmetabo_swm_n    = 0.0
       allocate(zoo(n)%wc_vert_int_jprod_n(isd:ied,jsd:jed))   ; zoo(n)%wc_vert_int_jprod_n      = 0.0
       allocate(zoo(n)%wc_vert_int_jprodpos_n(isd:ied,jsd:jed))   ; zoo(n)%wc_vert_int_jprodpos_n   = 0.0
       allocate(zoo(n)%wc_vert_int_jmort_n(isd:ied,jsd:jed))   ; zoo(n)%wc_vert_int_jmort_n      = 0.0
       allocate(zoo(n)%wc_vert_int_jzloss_n(isd:ied,jsd:jed))  ; zoo(n)%wc_vert_int_jzloss_n     = 0.0
       allocate(zoo(n)%wc_vert_int_jhploss_n(isd:ied,jsd:jed)) ; zoo(n)%wc_vert_int_jhploss_n    = 0.0
       allocate(zoo(n)%wc_vert_int_jzoo_n(isd:ied,jsd:jed))    ; zoo(n)%wc_vert_int_jzoo_n       = 0.0
       allocate(zoo(n)%wc_vert_int_f_n(isd:ied,jsd:jed))       ; zoo(n)%wc_vert_int_f_n          = 0.0
       allocate(zoo(n)%z_mig(isd:ied,jsd:jed))                 ; zoo(n)%z_mig                    = 0.0
       ! mpoupon - plankton dynamics
   enddo
  ...
  subroutine user_deallocate_arrays
    ...
    do n = 1, NUM_ZOO
       ...
       deallocate(zoo(n)%jingest_n_lim) ! mpoupon
    ...
    deallocate(cobalt%jnvmmdz_gut)  ! mpoupon
    deallocate(cobalt%jnvmlgz_gut)  ! mpoupon
    deallocate(cobalt%jpvmmdz_gut)  ! mpoupon
    deallocate(cobalt%jpvmlgz_gut)  ! mpoupon
    deallocate(cobalt%jfevmmdz_gut) ! mpoupon
    deallocate(cobalt%jfevmlgz_gut) ! mpoupon
    deallocate(cobalt%jsivmmdz_gut) ! mpoupon
    deallocate(cobalt%jsivmlgz_gut) ! mpoupon
    deallocate(cobalt%jnvmmdz_met)  ! mpoupon
    deallocate(cobalt%jnvmlgz_met)  ! mpoupon
    ...
    deallocate(cobalt%jprod_nh4_conso)    ! mpoupon
    deallocate(cobalt%jprod_nh4_resp)     ! mpoupon
    deallocate(cobalt%jprod_nh4_passive)  ! mpoupon
    deallocate(cobalt%jprod_ndet_passive) ! mpoupon
    deallocate(cobalt%jprod_don_passive)  ! mpoupon
    deallocate(cobalt%jprod_nh4_active_respiration) ! mpoupon
    deallocate(cobalt%jprod_ndet_active_digestion)  ! mpoupon
    deallocate(cobalt%jprod_ndet_active_mortality)  ! mpoupon
    deallocate(cobalt%jprod_don_active_excretion)   ! mpoupon
    ...
    deallocate(cobalt%hp_jingest_n_active)  ! mpoupon - budget
    deallocate(cobalt%hp_jingest_n_passive) ! mpoupon - budget
    deallocate(cobalt%hp_jingest_p)
    deallocate(cobalt%hp_jingest_sio2)
    deallocate(cobalt%hp_jingest_fe)
    deallocate(cobalt%irr_inst)
    deallocate(cobalt%irr_inst_cw) ! mpoupon
    ...
    deallocate(cobalt%tot_layer_int_pic) ! mpoupon
...
!==============================================================================================================
    ...
    do n = 1, NUM_PHYTO
       ...
       ! mpoupon - plankton dynamics
       deallocate(phyto(n)%wc_vert_int_maxgrowth_lightlim_n)
       deallocate(phyto(n)%wc_vert_int_maxgrowth_nutlim_n)
       deallocate(phyto(n)%wc_vert_int_maxgrowth_n)
       deallocate(phyto(n)%wc_vert_int_jprod_n)
       deallocate(phyto(n)%wc_vert_int_jresp_n)
       deallocate(phyto(n)%wc_vert_int_jvirloss_n)
       deallocate(phyto(n)%wc_vert_int_jaggloss_n)
       deallocate(phyto(n)%wc_vert_int_jexuloss_n)
       deallocate(phyto(n)%wc_vert_int_jzloss_n)
       deallocate(phyto(n)%wc_vert_int_jphyto_n)
       deallocate(phyto(n)%wc_vert_int_f_n)
    enddo
    ...
    do n = 1, NUM_ZOO
       ...
       ! mpoupon - plankton dynamics
       deallocate(zoo(n)%wc_vert_int_jingest_n)
       deallocate(zoo(n)%wc_vert_int_jmetabo_n)
       deallocate(zoo(n)%wc_vert_int_jmetabo_bas_n)
       deallocate(zoo(n)%wc_vert_int_jmetabo_dig_n)
       deallocate(zoo(n)%wc_vert_int_jmetabo_swm_n)
       deallocate(zoo(n)%wc_vert_int_jprod_n)
       deallocate(zoo(n)%wc_vert_int_jprodpos_n)
       deallocate(zoo(n)%wc_vert_int_jmort_n)
       deallocate(zoo(n)%wc_vert_int_jzloss_n)
       deallocate(zoo(n)%wc_vert_int_jhploss_n)
       deallocate(zoo(n)%wc_vert_int_jzoo_n)
       deallocate(zoo(n)%wc_vert_int_f_n)
       deallocate(zoo(n)%z_mig)
       ! mpoupon - plankton dynamics
    enddo
