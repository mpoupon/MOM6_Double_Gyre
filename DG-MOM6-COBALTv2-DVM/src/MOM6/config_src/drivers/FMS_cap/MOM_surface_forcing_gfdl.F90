  ...
  if (do_ustar .or. do_gustless) then
    ...
    if (associated(IOB%stress_mag)) then
      if (do_ustar) then ; do j=js,je ; do i=is,ie
        ...
        ustar(i,j) = 0.01 ! mpoupon ! sqrt(gustiness*IRho0 + IRho0*Pa_conversion*IOB%stress_mag(i-i0,j-j0))
      enddo ; enddo ; endif
      if (CS%answer_date < 20190101) then
        if (do_gustless) then ; do j=js,je ; do i=is,ie
          gustless_ustar(i,j) =  0.01 ! mpoupon ! sqrt(Pa_conversion*US%L_to_Z*IOB%stress_mag(i-i0,j-j0) / CS%Rho0)
        enddo ; enddo ; endif
      else
        if (do_gustless) then ; do j=js,je ; do i=is,ie
          gustless_ustar(i,j) =  0.01 ! mpoupon ! sqrt(IRho0 * Pa_conversion*IOB%stress_mag(i-i0,j-j0))
        enddo ; enddo ; endif
      endif
    elseif (wind_stagger == BGRID_NE) then
      do j=js,je ; do i=is,ie
        ...
        if (do_ustar) ustar(i,j) =  0.01 ! mpoupon !sqrt(gustiness*IRho0 + IRho0 * tau_mag)
        if (CS%answer_date < 20190101) then
          if (do_gustless) gustless_ustar(i,j) =  0.01 ! mpoupon !sqrt(US%L_to_Z*tau_mag / CS%Rho0)
        else
          if (do_gustless) gustless_ustar(i,j) =  0.01 ! mpoupon ! sqrt(IRho0 * tau_mag)
        endif
      enddo ; enddo
    elseif (wind_stagger == AGRID) then
      do j=js,je ; do i=is,ie
